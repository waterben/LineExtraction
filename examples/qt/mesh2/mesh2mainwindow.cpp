...
Mesh2MainWindow::Mesh2MainWindow(QWidget* parent) : DummyBase(parent) {
  ...
  // connect(datacolordlg_, SIGNAL(filesSelected(const QStringList&)), this, SLOT(adaptDataColors4(const
  // QStringList&)));
  ...
  if (!pmhandler) pmhandler = (PixmapWriter*)IO::outputHandler("jpeg");  // Qt4 naming scheme change
...
void Mesh2MainWindow::dumpImage() {
  ...
  if (vw) vw->setSortMode(VectorWriter::BSPSORT);
...
void Mesh2MainWindow::rotate() {
  ...
  dataWidget->setRotation(int(dataWidget->xRotation() + 1) % 360, int(dataWidget->yRotation() + 1) % 360,
                          int(dataWidget->zRotation() + 1) % 360);
}

void Mesh2MainWindow::toggleProjectionMode(bool val) { dataWidget->setOrtho(val); }
...
void Mesh2MainWindow::showRotate(double x, double y, double z) {
  rotateLabel->setText(" Angles (" + QString::number(x, 'g', 3) + " ," + QString::number(y, 'g', 3) + " ," +
...
void Mesh2MainWindow::showZoom(double z) { zoomLabel->setText(" Zoom " + QString::number(z, 'g', 3)); }
