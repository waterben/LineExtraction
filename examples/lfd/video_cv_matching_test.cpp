...
int main(int argc, char** argv)
{
    ...
    MotionLineFilter<MyType, LsdCC<MyType>::LineSegmentVector> mlf(200, 20, im_grayLeft.size().width, im_grayLeft.size().height);
...
*/
        ...
        //StereoLineMatcher<MyType, LsdCC<MyType>::LineSegmentVector,MyGchHelper> slm(fdcL, fdcR, im_grayLeft.rows, im_grayLeft.cols / 2,5,0.6);
        ...
        bd->detect( im_grayLeft, keylines1 );
        bd->detect( im_grayRight, keylines2 );

        /* compute descriptors */
        cv::Mat descr1, descr2;
        bd->compute( im_grayLeft, keylines1, descr1 );
        bd->compute( im_grayRight, keylines2, descr2 );
...
/*
        std::vector<lsfm::LineSegment<myFT>> leftLines, rightLines;

        leftLines = lsfm::keyLine2LineSegment<myFT>(mvLKeys);
        rightLines = lsfm::keyLine2LineSegment<myFT>(mvLKeysRight);
*/
        /* require match */
        std::vector<DMatch> lbdMatches;
        bdm->match( descr1, descr2, lbdMatches );
/*
        MyType mean = 0;
        ...
        imshow("Detected Stereo matches", drawMatches<MyType,DescriptorMatch<MyType>>(im_grayLeft, lsd1.lineSegments(), im_grayRight, lsd2.lineSegments(), bfmatches2));    
*/

        imshow("Detected matches, cv3 lbd matching", drawKeyLineMatches<FT,DescriptorMatch<FT>>(im_grayLeft, keylines1, im_grayRight, keylines2, lbdMatches));
...
/*
        ...
        std::cout << "width: " << im_grayLeft.size().width << " height: " << im_grayLeft.size().height << std::endl;
        MotionLineMatcher<MyType, LsdCC<MyType>::LineSegmentVector,MyMotionFdc> mlm(fdcM1, fdcM2, im_grayLeft.cols,  im_grayLeft.rows);
*/
...
*/
        ...
        imshow("Detected tracking matches", drawMatches<MyType,DescriptorMatch<MyType>>(im_grayLeft, lsd1.lineSegments(), previousImGrayLeft, previousLeftLines, motionMatches));
