...
int main(int argc, char** argv) {
  ...
  FdcLBD<MyFloat, LsdCC<MyFloat>::LineSegment, short, FastRoundNearestInterpolator<MyFloat, short>> fdc1(
      lsd1.imageData()[0], lsd1.imageData()[1], 7, 5);
  FdcLBD<MyFloat, LsdCC<MyFloat>::LineSegment, short, FastRoundNearestInterpolator<MyFloat, short>> fdc2(
      lsd2.imageData()[0], lsd2.imageData()[1], 7, 5);
  ...
  std::cout << "time for lbd bf radius: " << (end - start) * 1000 / getTickFrequency()
            << ", candidates: " << bfmatchesR.size() << std::endl;
  ...
  std::cout << "time for lbd pairwise matcher with bfmatcher: " << (end - start) * 1000 / getTickFrequency()
            << std::endl;
  ...
  imshow("Detected matches mbfmatches - schlecht",
         drawKeyLineMatches<MyFloat, DescriptorMatch<MyFloat>>(src1, klV1, src2, klV2, mbfmatches));
