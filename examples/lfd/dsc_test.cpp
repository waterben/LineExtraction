...
int main(int argc, char** argv)
{
    ...
    StereoLineMatcher<MyFloat, LsdCC<MyFloat>::LineSegmentVector,MyGchHelper> slm(fdcL, fdcR, srcL.rows, srcL.cols / 2,5,0.6);
    ...
    std::cout <<  "time for stereo filter: " << (end - start) * 1000 / cv::getTickFrequency() << "ms, candidates: " << bfmatchesT.size() << std::endl;

    start = double(cv::getTickCount());
    slf.create(lsdL.lineSegments(), lsdR.lineSegments(), bfmatchesT2, maskL2, maskR2);
    end = double(cv::getTickCount());
    std::cout <<  "time for stereo filter create: " << (end - start) * 1000 / cv::getTickFrequency() << "ms, candidates: " << bfmatchesT2.size() << std::endl;
    ...
    std::cout <<  "slm matches: " << bfmatches3.size() << ", time for matching: " << (end - start) * 1000 / cv::getTickFrequency() << std::endl;
    ...
    imshow("Detected matches", drawMatches<MyFloat,DescriptorMatch<MyFloat>>(srcL, lsdL.lineSegments(), srcR, lsdR.lineSegments(), bfmatches2));
