...
*/
    ...
    std::cout << "Processing Time lineDetection on first image in s: " <<  std::chrono::duration_cast<std::chrono::duration<double> >(detect2 - detect1).count() << std::endl;
    ...
    std::cout << "Processing Time matching in s: " <<  std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1).count() << std::endl;
    std::cout << "matches: " << lsd_matches.size() << std::endl;

    /* select best matches */
    std::vector<DMatch> removed_maches, filtered_matches2 = postfilterStereoMatches(lsd_matches, slf, leftLines, rightLines, removed_maches);
 /*
...
/*
    imshow("Detected matches, cv3 lbd matching", drawKeyLineMatches<MyFloat,DescriptorMatch<MyFloat>>(srcL, octave0_1, srcR, octave0_2, lsd_matches));
    imshow("Detected matches filtered2, cv3 lbd matching", drawKeyLineMatches<MyFloat,DescriptorMatch<MyFloat>>(srcL, octave0_1, srcR, octave0_2, filtered_matches2));
    imshow("Detected matches removed_maches, cv3 lbd matching", drawKeyLineMatches<MyFloat,DescriptorMatch<MyFloat>>(srcL, octave0_1, srcR, octave0_2, removed_maches));
*/
...
/*    cv::Mat lsd_outImg;
    ...
    drawLineMatches( srcL, octave0_1, srcR, octave0_2, good_matches, lsd_outImg, Scalar::all( -1 ), Scalar::all( -1 ), lsd_mask,
