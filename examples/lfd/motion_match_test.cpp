...
int main(int argc, char** argv) {
  ...
  std::cout << "candidates: " << matches.size()
            << ", time for MotionLineFilter create: " << (end - start) * 1000 / getTickFrequency() << "ms" << std::endl;
  std::cout << "lines left: " << lsd1.lineSegments().size() << ", lines right: " << lsd2.lineSegments().size()
            << std::endl;
  // size_t num = matches.size();
  size_t num = 0;
  for_each(matches2D.begin(), matches2D.end(), [&num](const std::vector<FeatureMatch<MyType>>& m) { num += m.size(); });
  ...
  imshow(
      "Detected matches, Pairwise & Motion Line Filter - nicht so gut",
      drawMatches<MyType, DescriptorMatch<MyType>>(src1, lsd1.lineSegments(), src2, lsd2.lineSegments(), bfmatchesRes));
