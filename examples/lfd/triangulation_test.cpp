...
int main(int argc, char** argv) {
  ...
  imL = frames(cv::Range(0, frames.size().height),
               cv::Range(0, frames.size().width /
                                2));  // no data copying here - Range excludes the last value of width, thus correct.
  imR = frames(cv::Range(0, frames.size().height),
               cv::Range(frames.size().width / 2, frames.size().width));  // no data copying here

  cv::Mat img1r, img2r;
  remap(imL, img1r, map11, map12, INTER_LINEAR);  // Undistort
  remap(imR, img2r, map21, map22, INTER_LINEAR);

  imL = img1r(Rect(croppingWidth, croppingHeight, img1r.size().width - croppingWidth * 2,
                   img1r.size().height - croppingHeight * 2))
            .clone();
  imR = img2r(Rect(croppingWidth, croppingHeight, img2r.size().width - croppingWidth * 2,
  ...
  while (last_key_press != 'q') {
    ...
    */
    ...
    StereoLineMatcher<MyType, LsdCC<MyType>::LineSegmentVector, MyGchHelper> slm(fdcL, fdcR, imL.rows, imL.cols / 2, 5,
                                                                                 0.6);
    ...
    imshow("Detected Stereo matches", drawMatches<MyType, DescriptorMatch<MyType>>(imL, lsdL.lineSegments(), imR,
    ...
    for_each(bfmatches2.begin(), bfmatches2.end(), [&](const DescriptorMatch<MyType>& match) {
      ...
      std::cout << "3dCV  - origin: " << l3sCV.origin() << ", dir: " << l3sCV.direction()
                << ", start: " << l3sCV.startPoint() << ", end: " << l3sCV.endPoint() << std::endl;
      std::cout << "3dpl  - origin: " << l3s.origin() << ", dir: " << l3s.direction() << ", start: " << l3s.startPoint()
                << ", end: " << l3s.endPoint() << std::endl;
      std::cout << "difference: dir: " << l3sCV.direction() - l3s.direction()
                << ", start: " << l3sCV.startPoint() - l3s.startPoint()
                << ", end: " << l3sCV.endPoint() - l3s.endPoint() << std::endl;

      std::cout << "left  - origin: " << lsL.origin() << ", dir: " << lsL.direction() << ", start: " << lsL.startPoint()
                << ", end: " << lsL.endPoint() << std::endl;
      std::cout << "right - origin: " << lsR.origin() << ", dir: " << lsR.direction() << ", start: " << lsR.startPoint()
                << ", end: " << lsR.endPoint() << std::endl;


      /*          prjLCV  = camCVL.project(l3sCV);
                std::cout << "projL - origin: " << prjLCV.origin() << ", dir: " << prjLCV.direction() << ", start: " <<
         prjLCV.startPoint() << ", end: " << prjLCV.endPoint() << std::endl; prjLCV  = camPlL.project(l3sCV); std::cout
         << "pPlüL - origin: " << prjLCV.origin() << ", dir: " << prjLCV.direction() << ", start: " <<
         prjLCV.startPoint() << ", end: " << prjLCV.endPoint() << std::endl; prjLCV  = camCVR.project(l3sCV); std::cout
         << "projR - origin: " << prjLCV.origin() << ", dir: " << prjLCV.direction() << ", start: " <<
         prjLCV.startPoint() << ", end: " << prjLCV.endPoint() << std::endl; prjLCV  = camPlR.project(l3sCV); std::cout
         << "pPlüR - origin: " << prjLCV.origin() << ", dir: " << prjLCV.direction() << ", start: " <<
         prjLCV.startPoint() << ", end: " << prjLCV.endPoint() << std::endl;

                prjL  = camCVL.project(l3s);
                std::cout << "projL - origin: " << prjL.origin() << ", dir: " << prjL.direction() << ", start: " <<
         prjL.startPoint() << ", end: " << prjL.endPoint() << std::endl; prjL  = camPlL.project(l3s); std::cout <<
         "pPlüL - origin: " << prjL.origin() << ", dir: " << prjL.direction() << ", start: " << prjL.startPoint() << ",
         end: " << prjL.endPoint() << std::endl; prjL  = camCVR.project(l3s); std::cout << "projR - origin: " <<
         prjL.origin() << ", dir: " << prjL.direction() << ", start: " << prjL.startPoint() << ", end: " <<
         prjL.endPoint() << std::endl; prjL  = camPlR.project(l3s); std::cout << "pPlüR - origin: " << prjL.origin() <<
         ", dir: " << prjL.direction() << ", start: " << prjL.startPoint() << ", end: " << prjL.endPoint() <<
         std::endl;*/
    ...
    std::cout << "lines left: " << lsdL.lineSegments().size() << ", lines right: " << lsdR.lineSegments().size()
              << std::endl;
    // size_t num = matches.size();
    size_t num = matches.size();

    std::cout << "reduced candidates from " << lsdL.lineSegments().size() * lsdR.lineSegments().size() << " to " << num
              << std::endl;
    ...
    */
    ...
    imshow("Detected tracking matches",
           drawMatches<MyType, DescriptorMatch<MyType>>(imL, lsdL.lineSegments(), previousImGrayLeft, previousLeftLines,
                                                        motionMatches));
