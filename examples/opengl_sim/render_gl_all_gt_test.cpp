...
//    tmpPose2.rotate(cam0Pose.rotM(), lsfm::Vec3<FT>(0,0,0));
    ...
    lsfm::Vec3<FT> trans0 = lsfm::Vec3<FT>(lsfm::Vec3<FT>(-rodrigues(cam0PoseCV.orientation()).transpose()*cam0PoseCV.origin()).data());
    lsfm::Vec3<FT> r0 = -cam0PoseCV.orientation();
    lsfm::Pose<FT> hMatPose0(trans0, r0);
    lsfm::Vec3<FT> trans1 = lsfm::Vec3<FT>(lsfm::Vec3<FT>(-rodrigues(cam1PoseCV.orientation()).transpose()*cam1PoseCV.origin()).data());
    lsfm::Vec3<FT> r1 = -cam1PoseCV.orientation();
    lsfm::Pose<FT> hMatPose1(trans1, r1);


    std::vector<lsfm::LineSegment3<FT>> gtLineModels, visibleGtLineModels0, visibleGtLineModels1;
    std::vector<int> lineIds0, lineIds1, lineIds0Vis, lineIds1Vis;
    for(int i = 0; i < endPoints.size(); ++i){
        gtLineModels.push_back(lsfm::LineSegment3<FT>(endPoints[i].first, endPoints[i].second));
        lineIds0.push_back(i);
        lineIds1.push_back(i);
    }

    visibilityCheck(gtLineModels, visibleGtLineModels0, hMatPose0.homM(), lineIds0, lineIds0Vis);
    visibilityCheck(gtLineModels, visibleGtLineModels1, hMatPose1.homM(), lineIds1, lineIds1Vis);

    lsfm::CameraPluecker<FT> cam0gtP(cam0gt), cam1gtP(cam1gt);
    std::vector<lsfm::LineSegment2<FT>> lsList0, lsList1;
    for(int i = 0; i < visibleGtLineModels0.size(); ++i){
        lsfm::LineSegment2<FT> ls0 = cam0gtP.projectLineSegment(cam0gtP.projM(), visibleGtLineModels0[i]);
        lsList0.push_back(ls0);
    }
    for(int i = 0; i < visibleGtLineModels1.size(); ++i){
        lsfm::LineSegment2<FT> ls1 = cam1gtP.projectLineSegment(cam1gtP.projM(), visibleGtLineModels1[i]);
        lsList1.push_back(ls1);
    }


    lsfm::lineProcessingAllGt<FT, MySlam, MapType>(lsList0, lsList1, lineIds0Vis, lineIds1Vis, cam0img, cam1img, cam0GtImg, cam1GtImg, gtToModelId, slam, currentPoseEstimation, cam0gt, cam1gt);
...
_Matrix_Type_ pseudoInverse(const _Matrix_Type_ &a, T epsilon = lsfm::LIMITS<FT>::tau())
{
            ...
            for(int j = -1; j <= 1; ++j)
                for(int k = -1; k <= 1; ++k)
                    for(int l = -1; l <= 1; ++l)
                        for(int m = -1; m <= 1; ++m)
                            for(int n = -1; n <= 1; ++n){
                                lsfm::Line3<FT> tl03(lsfm::Vec3<FT>(static_cast<FT>(i),static_cast<FT>(j), static_cast<FT>(k)),lsfm::Vec3<FT>(static_cast<FT>(l),static_cast<FT>(m),static_cast<FT>(n)));
                                lsfm::Vec4<FT> testCayley = tl03.cayley();
                                lsfm::Line3<FT> tl0 = lsfm::Line3<FT>::lineFromCayley(testCayley);

                                FT d1 = tl03.distance(lsfm::Vec3<FT>(0,0,-1)) - tl0.distance(lsfm::Vec3<FT>(0,0,-1));
                                FT d2 = tl03.distance(lsfm::Vec3<FT>(1,1,1)) - tl0.distance(lsfm::Vec3<FT>(1,1,1));
                                if(d1 > 0.01 || d2 > 0.01)
                                std::cout << " dist1: " << d1 << " dist 2: " << d2 <<
                                             "  " << i << "  " << j << "  " << k << "  " << l << "  " << m << "  " << n << "  " << std::endl;
                            }
*/
        lsfm::LineSegment<FT> lsL(0.42048965917666908, -0.90729733082682917, 241.29575630155182, -789.84932312428612, -461.46234108290207);
        lsfm::LineSegment<FT> lsR(0.4416954088398109, -0.89716507166174964, 230.5654696710223, -724.5983471053388, -411.97718919170637);

        lsfm::Camera<FT> triangulationCamLeft(651.83766184073568, 651.83766184073568, 480, 270, 960, 540, -9.7709937158538391, 3.1988025830389435, -16.716395348563047, 2.1535948769092332, 0.42834371405228522, -1.0042377836661469);
        lsfm::Camera<FT> triangulationCamRight(651.83766184073568, 651.83766184073568, 480, 270, 960, 540, -9.5781574329478776, 3.1988025830389435, -16.946208681498742, 2.1535948769092332, 0.42834371405228522, -1.0042377836661469);
        lsfm::StereoPlane<FT> modelStereo(triangulationCamLeft, triangulationCamRight);
        lsfm::LineSegment3<FT> modelLine3 = modelStereo.triangulate(lsL, lsR);

        FT err = 0;
        slam->reprojectionError(lsfm::CameraPluecker<FT>(triangulationCamLeft), modelLine3, lsL, err);
        slam->reprojectionError(lsfm::CameraPluecker<FT>(triangulationCamRight), modelLine3, lsR, err);
