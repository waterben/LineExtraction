...
int main(int argc, char** argv)
{
    ...
    const uchar outliers = (FeaturePoints<FT>::OUTLIER_DIST | FeaturePoints<FT>::OUTLIER_RANSAC);
...
//    typedef lsfm::Line3d MyLine3D;
    ...
    LsdEL_tracker<FT> lsd1(0.004, 0.012, 60, 3, 30, 10, EL_USE_NFA);
...
//    cv::equalizeHist(tmpImg2, prevImLeft);
    ...
    FeaturePoints<FT> features(FeaturePoints<FT>::MIN_NUM_FEATURES, 40.0, 5, 5, 100.0, 7.0, 0.6);
    ...
    LineMatcher<FT> matcher(10.0, 15.0, 20.0, LineMatcher<FT>::MATCH_RELATIVE);
    matcher.init(lines);
...
//        imRight = frameLR(cv::Range(0, frameLR.rows), cv::Range(frameLR.cols/2 + 1, frameLR.cols));
        ...
        features.outlierDetection(outliers);

        // line detection ---------------------------------------
        int64 dstart = cv::getTickCount();

        lsd1.processEdges(imLeftTransf);
        lsd1.detectLineSplitSeed(lsd1.grad(), lsd1.nms(), features, 3, outliers);
...
//        histogram(imLeft, eHist);
        ...
        // match lines ----------------------------------------------------------------
        uchar* linesMask = new uchar[lines.size()];
        //uchar* linesMask = 0;
        matcher.match(features, lastLines, lines, 0, linesMask);
...
//        cv::Mat linesLeft = imLeft.clone();
        ...
        //draw lines
        matcher.drawNextMatchLines(dirMap, lines, Scalar(255,255,255));
        matcher.drawPrevMatchLines(linesPrevLeft, lastLines, Scalar(255,0,0));
...
//        }
        //draw points
        //features.drawFeaturePoints(linesPrevLeft, linesMask);
        //features.drawValidPredPoints(dirMap, outliers);
...
//        drawHist(eHistImg, eHist);
        ...
        features.calcFeaturePoints(lines, 0);
...
//        lastHist = hist;
        ...
        //print summary
        cout<<"cycle: "<<cycleNr<<"\tll: "<<lastLines.size()<<" cl: "<<lines.size()<<"\tmatches: "<<matcher.numMatches()<<"\tmerror: " << error << "\tdtime: " << dtime << "\ttime: " << time << "\n";
//        for(int i=0; i<lines.size(); i++) {
