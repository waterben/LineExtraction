...
//    cv::equalizeHist(tmpImg2, prevImLeft);
    ...
    FeaturePoints<FT, PT, PT> features(tmpImg1.cols, FeaturePoints<FT, PT, PT>::MIN_NUM_FEATURES, 40.0, 5, 5, 100.0, 5.0, 0.75);
//    features.calcFeaturePoints(lsd1.lineSupportSegments(), lsd1.points());
    ...
    //myMatcher.init(lines.size(), prevImLeft.cols);
    ...
    while (last_key_press != 'q')
    {
        ...
        features.shiftToEdge(lsd1.nms().directionMap());
...
//        histogram(imLeft, eHist);
        ...
        //myMatcher.match<PT>(features.getLinePoints(), lsd1.nms().directionMap(), lsd1.edge().points(), lsd1.edge().segments());
