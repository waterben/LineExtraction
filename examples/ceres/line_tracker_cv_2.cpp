...
//    cv::equalizeHist(tmpImg2, prevImLeft);
    ...
    LineMatcher<FT> matcher(10.0, 15.0, 120.0, LineMatcher<FT>::MATCH_RELATIVE);
    matcher.init(lines);
...
//        lsd1.detectLineSegments(lsd1.grad(), lsd1.nms(), features, FeaturePoints<FT>::OUTLIER_DIST | FeaturePoints<FT>::OUTLIER_RANSAC);
//        LineSegment2Vector<FT> Testlines = lsd1.lineSegments();

        vector<cv::Point2f> pts;
        features.getValidPredPoints(pts, outliers);
...
//        //std::cout<<"size of seeds: "<<seeds.size()<<"\tpredicted points: "<<pts.size()<<"\tseedcounter: "<<seedcounter<<"\n";
        ...
        lsd1.detectLines(lsd1.grad(), lsd1.nms(), seeds);
