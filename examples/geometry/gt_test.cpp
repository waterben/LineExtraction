...
int main() {
  ...
  lsfm::Vec3<FT> orientation(
      rodrigues(lsfm::Matx33<FT>(rodrigues(lsfm::Vec3<FT>(0, 0, CV_PI / 2)) * rodrigues(lsfm::Vec3<FT>(CV_PI, 0, 0)))));
  ...
  std::cout << "rot: " << camLCV.orientation() << ", q: " << q << ", rrot: " << rvec << ", rq: " <<
  quaterion(rodrigues(rvec)) << std::endl;

  lsfm::Vec3<FT> e = euler(camLCV.rotM());
  rvec = rodrigues(euler(e));
  std::cout << "rot: " << camLCV.orientation() << ", e: " << e << ", rrot: " << rvec << ", re: " <<
  euler(rodrigues(rvec))  << std::endl;*/
