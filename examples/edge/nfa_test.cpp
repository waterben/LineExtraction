...
inline void setPixel(cv::Mat &dst, cv::Point idx, const Vec3b &color) {
    dst.at<cv::Vec3b>(idx)= color;
}

inline void setPixel(cv::Mat &dst, lsfm::Vec3i idx, const Vec3b &color) {
...
void showNfa(EDGE &edge, const cv::Mat &src, const NFA &nfa, const std::string &name, bool circles = true) {
    ...
    for_each(edge.segments().begin(), edge.segments().end(), [&](const EdgeSegment &seg) {
        for (size_t i = seg.begin(); i != seg.end(); ++i)
            setPixel(edgeImg, edge.points()[i], removed);
    });
