...
namespace lsfm {
    ...
    public:
        static void logGaborFilter(cv::Mat *lgf, cv::Mat &H, int rows, int cols, FT minW, FT mult, FT sigmaOnf, int nscale, FT cutoff = static_cast<FT>(0.45), int n = 15) {
            cv::Mat u1, u2, r;
    ...
    private:
        ...
        void init() {
            ...
            this->add("grad_minWaveLength", std::bind(&PCLgf::minWaveLength, this, std::placeholders::_1), "Minimal wave length.");
            ...
            this->add("grad_noiseMethod", std::bind(&PCLgf::noiseMethod, this, std::placeholders::_1), "Noise method: 0 = none, -1 = dsdf, -2 = sdf, > 0 = threshold.");
            max_response();
        }
	...
	public:
        ...
        PCLgf(int nscale = 4, FT minWaveLength = 3, FT mult = static_cast<FT>(2.1),
            FT sigmaOnf = static_cast<FT>(0.55), FT k = 3, FT cutOff = static_cast<FT>(0.5), FT g = 10,
            FT deviationGain = static_cast<FT>(1.5), double noiseMethod = -1, 
            img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max()) 
            : PhaseCongruency<IT,FT,FT,FT,FT>(int_lower, int_upper),
            rows_(0), cols_(0), rows_ext_(0), cols_ext_(0), nscale_(nscale), minW_(minWaveLength), mult_(mult), sigmaOnf_(sigmaOnf), k_(k), cutOff_(cutOff), g_(g), deviationGain_(deviationGain),
            noiseMethod_(noiseMethod), T_(0), eps_(static_cast<FT>(0.0001)) {
            init();
        }

        PCLgf(const ValueManager::NameValueVector &options, img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max())
            : PhaseCongruency<IT, FT, FT, FT, FT>(int_lower, int_upper), rows_(0), cols_(0), rows_ext_(0), cols_ext_(0), nscale_(4), minW_(3), mult_(static_cast<FT>(2.1)), 
            sigmaOnf_(static_cast<FT>(0.55)), k_(3), cutOff_(static_cast<FT>(0.5)), g_(10), deviationGain_(1.5), noiseMethod_(-1), T_(0), eps_(static_cast<FT>(0.0001)) {
            init();
            value(options);
        }

        PCLgf(ValueManager::InitializerList options, img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max())
            : PhaseCongruency<IT, FT, FT, FT, FT>(int_lower, int_upper), rows_(0), cols_(0), rows_ext_(0), cols_ext_(0), nscale_(4), minW_(3), mult_(static_cast<FT>(2.1)), 
            sigmaOnf_(static_cast<FT>(0.55)), k_(3), cutOff_(static_cast<FT>(0.5)), g_(10), deviationGain_(1.5), noiseMethod_(-1), T_(0), eps_(static_cast<FT>(0.0001)) {
            init();
            value(options);
        ...
        Value numScales(const Value &ns = Value::NAV()) { if (ns.type()) {nscale_ = ns; max_response(); } return nscale_;}
        Value minWaveLength(const Value &mw = Value::NAV()) { if (mw.type()) {minW_ = mw; max_response(); } return minW_;}
        Value mult(const Value &m = Value::NAV()) { if (m.type()) {mult_ = m; max_response(); } return mult_;}
        Value sigmaOnf(const Value &so = Value::NAV()) { if (so.type()) {sigmaOnf_ = so; max_response(); } return sigmaOnf_;}
        ...
        Value deviationGain(const Value &g = Value::NAV()) { if (g.type()) deviationGain_ = g; return deviationGain_;}
        Value noiseMethod(const Value &n = Value::NAV()) { if (n.type()) noiseMethod_ = n; return noiseMethod_;}
