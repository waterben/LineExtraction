...
namespace lsfm {
        ...
        static cv::Mat_<FT> createFilter(int rows, int cols, FT spacing, FT s, FT m, KernelType f) {
            ...
            for (int i = 0; i != rows; ++i)
                for (int j = 0; j != cols; ++j)
                    kernel(i,j) = f((j-starty)/step, (i-startx)/step, s, m);
    ...
    protected:
        ...
        void init() {
            this->add("grad_kernel_spacing", std::bind(&QuadratureSF<IT, FT, P>::valueKernelSpacing, this, std::placeholders::_1), "Spacing for a single step for Quadrature-Operators.");
            this->add("grad_scale", std::bind(&QuadratureSF<IT, FT, P>::valueScale, this, std::placeholders::_1), "Coarsest scale for filter.");
            this->add("grad_muls", std::bind(&QuadratureSF<IT, FT, P>::valueMuls, this, std::placeholders::_1), "Multiply for coarsed scale.");
    ...
    public:
        ...
        QuadratureSF(FT scale = 1, FT muls = 2, FT kernel_spacing = 1, IT int_lower = std::numeric_limits<IT>::lowest(), IT int_upper = std::numeric_limits<IT>::max())
            : Quadrature<IT, FT, FT, FT, FT>(int_lower, int_upper), kspacing_(kernel_spacing), scale_(scale), muls_(muls), cols_(0), cols_ext_(0), rows_(0), rows_ext_(0) {
            init();
        }

        QuadratureSF(FT scale, FT l, int k, FT kernel_spacing, IT int_lower = std::numeric_limits<IT>::lowest(), IT int_upper = std::numeric_limits<IT>::max())
            : Quadrature<IT, FT, FT, FT, FT>(int_lower, int_upper), kspacing_(kernel_spacing), scale_(scale*pow(l, k)), muls_(pow(l, k - 1) / pow(l, k)), cols_(0), cols_ext_(0), rows_(0), rows_ext_(0) {
            init();
        }

        QuadratureSF(const ValueManager::NameValueVector &options, img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max())
            : Quadrature<IT, FT, FT, FT, FT>(int_lower, int_upper), kspacing_(1), scale_(1), muls_(2), cols_(0), cols_ext_(0), rows_(0), rows_ext_(0) {
            init();
            value(options);
        }

        QuadratureSF(ValueManager::InitializerList options, img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max())
            : Quadrature<IT, FT, FT, FT, FT>(int_lower, int_upper), kspacing_(1), scale_(1), muls_(2), cols_(0), cols_ext_(0), rows_(0), rows_ext_(0) {
            init();
            value(options);
        }
