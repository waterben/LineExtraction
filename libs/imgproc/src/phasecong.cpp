...
// Return Type  : void
//
void phasecong(const emxArray_real_T* im,
               const emxArray_real_T* lgf,
               const emxArray_creal_T* H,
               double mult,
               double k,
  ...
  while (s <= lgf->size[2] - 1) {
    ...
    for (i9 = 0; i9 < n; i9++) {
      ...
      for (b_k = 0; b_k < loop_ub; b_k++) {
        IMF->data[b_k + IMF->size[0] * i9].re = lgf->data[(b_k + lgf->size[0] * i9) + lgf->size[0] * lgf->size[1] * s] *
                                                IM->data[b_k + IM->size[0] * i9].re;
        IMF->data[b_k + IMF->size[0] * i9].im = lgf->data[(b_k + lgf->size[0] * i9) + lgf->size[0] * lgf->size[1] * s] *
                                                IM->data[b_k + IM->size[0] * i9].im;
      }
    }
  ...
  if (noiseMethod >= 0.0) {
    ...
    //  geometric sum.
    totalTau = tau * (1.0 - rt_powd_snf(1.0 / mult, (double)lgf->size[2])) / (1.0 - 1.0 / mult);
  ...
  do {
    ...
    if (k <= high_i - 1) {
      ...
      if ((!(edges->size[1] == 0)) && (!rtIsNaN(data->data[xind]))) {
        if ((data->data[xind] >= edges->data[0]) && (data->data[xind] < edges->data[edges->size[1] - 1])) {
          nm1d2 = 1;
          ixstart = 2;
          high_i = edges->size[1];
