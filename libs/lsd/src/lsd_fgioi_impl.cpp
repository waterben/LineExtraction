...
static image_double gaussian_sampler(image_double in, double scale, double sigma_scale) {
  ...
  /* check parameters */
  if (in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0) error("gaussian_sampler: invalid image.");
...
static image_double ll_angle(image_double in,
  ...
  /* check parameters */
  if (in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0) error("ll_angle: invalid image.");
...
static inline double log_gamma(double x) { return x > 15 ? log_gamma_windschitl(x) : log_gamma_lanczos(x); }
...
static double inter_low(double x, double x1, double y1, double x2, double y2) {
  /* check parameters */
  if (x1 > x2 || x < x1 || x > x2) error("inter_low: unsuitable input, 'x1>x2' or 'x<x1' or 'x>x2'.");
...
static double get_theta(
    struct point* reg, int reg_size, double x, double y, image_double modgrad, double reg_angle, double prec) {
 ...
 */
static void region2rect(
    struct point* reg, int reg_size, image_double modgrad, double reg_angle, double prec, double p, struct rect* rec) {
  ...
  */
  ...
  for (; list_p != NULL; list_p = list_p->next)
       ...
       because we are only interested in the exact NOTDEF value */
    {
       ...
       */
      if (!refine(reg, &reg_size, modgrad, reg_angle, prec, p, &rec, used, angles, density_th)) continue;
...
double* lsd_scale_region(int* n_out, double* img, int X, int Y, double scale, int** reg_img, int* reg_x, int* reg_y) {
  ...
  return LineSegmentDetection(n_out, img, X, Y, scale, sigma_scale, quant, ang_th, log_eps, density_th, n_bins, reg_img,
                              reg_x, reg_y);
}
