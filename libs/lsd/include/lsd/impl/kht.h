...
class accumulator_t {
 ...
 public:
  ...
  inline int** bins() { return m_bins; }

  // Returns the accumulator bins ([1,height][1,width] range).
  inline const int** bins() const { return const_cast<const int**>(m_bins); }

  // Set zeros to the accumulator bins.
  inline void clear() { memset_2d(m_bins, 0, m_height + 2, m_width + 2, sizeof(int)); }
  ...
  inline size_t image_height() const { return m_image_height; }

  // Returns the expected image width.
  inline size_t image_width() const { return m_image_width; }

  // Initializes the accumulator.
  inline void init(const size_t image_width, const size_t image_height, const double delta) {
  ...
  inline const double* rho() const { return m_rho; }

  // Returns the parameters space limits (rho dimention, in pixels).
  inline const bounds_t& rho_bounds() const { return m_rho_bounds; }
  ...
  inline const bounds_t& theta_bounds() const { return m_theta_bounds; }
