...
namespace lsfm {
	...
	struct LinearEstimate {
		...
		static inline FT estimate(MT p, MT p_m, MT p_p) {
			return (p_p - p_m) / (2 * (p - std::min(p_m, p_p)));
		}
	};
	...
	struct CoGEstimate {
		...
		static inline FT estimate(MT p, MT p_m, MT p_p) {
			return (p_p - p_m) / (p + p_p + p_m - 3 * std::min(p_m,p_p));
		}
	};
	...
	template <class FT, class MT, template<class> class PT = Vec2, template<class, class> class Estimator = QuadraticEstimate, template<class, class> class Interpolator = LinearInterpolator>
	struct SubPixelEstimator {
		...
		static inline void estimate(const Pointf& p, const Pointf& dir, const cv::Mat& mag, Pointf& res) {
			res = p + dir * estimate(interpolator_type::getNB(mag, p), interpolator_type::getNB(mag, p-dir), interpolator_type::getNB(mag, p+dir));
		}
		...
		static inline void estimate(const Pointf &p, const Pointf &p_m, const Pointf &p_p, const cv::Mat &mag, Pointf &res) {
			res = p + (p_p - p) * estimate(interpolator_type::getNB(mag, p), interpolator_type::getNB(mag, p_m), interpolator_type::getNB(mag, p_p));
		}
		...
		static inline void estimate(index_type p, const Pointf &dir, const cv::Mat& mag, Pointf &res) {
			...
			Pointf pf;
            index2Point(p, pf, mag.cols);
			res = pf + dir * estimate(mag.ptr<MT>()[p], interpolator_type::getNB(mag, pf - dir), interpolator_type::getNB(mag, pf + dir));
		}
		...
		static inline void estimate(index_type p, FT dir, const cv::Mat& mag, Pointf &res) {
			...
			res = pf + dirf * estimate(mag.ptr<MT>()[p], interpolator_type::getNB(mag, pf - dirf), interpolator_type::getNB(mag, pf + dirf));
		}
		...
		static inline void estimate(const Pointi &p, FT dir, const cv::Mat& mag, Pointf &res) {
            ...
            res = pf + dirf * estimate(get<MT>(mag, p), interpolator_type::getNB(mag, pf - dirf), interpolator_type::getNB(mag, pf + dirf));
		}
		...
		static inline void estimate(const Pointf &p, FT dir, const cv::Mat& mag, Pointf &res) {
			...
			res = p + dirf * estimate(interpolator_type::getNB(mag, p), interpolator_type::getNB(mag, p - dirf), interpolator_type::getNB(mag, p + dirf));
		}
		...
		// Basic worker, applies the pyramid fitting equation
		static inline FT estimate(MT p, MT p_m, MT p_p){
			return estimator_type::estimate(p,p_m,p_p);
		}

        static inline void convertP(const Pointi *beg, const Pointi *end, Pointf *data, const cv::Mat& mag, const cv::Mat& dmap) {
            char dir;
        ...
        static inline void convertPDir(const Pointi *beg, const Pointi *end, Pointf *data, const cv::Mat& mag, const cv::Mat& dir) {
            FT d;
            for (; beg != end; ++beg, ++data) {
                d = get<FT>(dir, *beg);
        ...
        static inline void convert(const index_type *beg, const index_type *end, Pointf *data, const cv::Mat& mag, const cv::Mat& dmap) {
            char dir;
            for (; beg != end; ++beg, ++data) {
                dir = get<char>(dmap, *beg);
                if (dir < 0) {
        ...
        static inline void convertDir(const index_type *beg, const index_type *end, Pointf *data, const cv::Mat& mag, const cv::Mat& dir) {
            FT d;
            for (; beg != end; ++beg, ++data) {
                d = get<FT>(dir, *beg);
                estimate(*beg, d, mag, *data);
    ...
    struct PixelEstimator {
        ...
        static inline void convert(const index_type *beg, const index_type *end, point_type *data, const cv::Mat& mag, const cv::Mat& dmap) {
        ...
        static inline void convertDir(const index_type *beg, const index_type *end, point_type *data, const cv::Mat& mag, const cv::Mat& dir) {
