...
/// @brief Unit tests for camera models.
...
TEST(CameraTest, ComposeAndDecomposeProjectionMatrix) {
  ...
  Matx34f P = lsfm::Camera<float>::composeProjectionMatrix(lsfm::Camera<float>::composeCameraMatrix(focal, offset), t,
                                                           lsfm::rodrigues(r));
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 4; j++) EXPECT_NEAR(P(i, j) / s1, P2(i, j) / s2, 1e-3f);
}
