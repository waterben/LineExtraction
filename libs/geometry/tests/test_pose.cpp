...
TEST(PoseTest, OrientationAndHom)
{
  ...
  for (int i=0;i<4;i++) for (int j=0;j<4;j++) EXPECT_NEAR(H(i,j), Hd(i,j), 1e-5f);
}
...
TEST(PoseTest, ConcatAndConcatInverse)
{
  ...
  for (int i=0;i<4;i++) for (int j=0;j<4;j++) EXPECT_NEAR(actual(i,j), expected(i,j), 1e-5f);
  ...
  Matx44f p1H = p1.homM();
  for (int i=0;i<4;i++) for (int j=0;j<4;j++) EXPECT_NEAR(backH(i,j), p1H(i,j), 1e-5f);
}
