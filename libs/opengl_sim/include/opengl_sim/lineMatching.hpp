...
namespace lsfm{
    ...
    KeyLine lineSegment2KeyLine(const LineSegment<FT> &ls, const int & imgMaxSize, int classId = 0, int octave = 0){
        ...
        kl.size = static_cast<float>( std::abs(ls.startPoint().x() - ls.endPoint().x()) * std::abs(ls.startPoint().y() - ls.endPoint().y()) );
    ...
    std::vector<FeatureTypeA> postfilterStereoMatches(std::vector<FeatureTypeA> matches, std::vector<FeatureTypeB> filter, const int size1, const int size2){
    ...
    template<class FeatureTypeA, class GeoFilter, class GV>
    std::vector<FeatureTypeA> postfilterStereoMatches(std::vector<FeatureTypeA> matches, GeoFilter filter, GV l1, GV l2, std::vector<FeatureTypeA> & filtered){
        std::vector<FeatureTypeA> result;
    ...
    cv::Mat drawKeyLineMatches(const cv::Mat& img1, const GV& l1,
        const cv::Mat& img2, const GV& l2,
        const std::vector<DMatch>& matches, const std::vector<int> lineNumber = std::vector<int>(),
        bool connect = false) {
        ...
        for_each(matches.begin(), matches.end(), [&](const DMatch &m){
            ...
            LineSegment<FT,lsfm::Vec2> line1(lsfm::Vec4<FT>(lsfm::getX(l1[m.queryIdx].getStartPoint()), lsfm::getY(l1[m.queryIdx].getStartPoint()), lsfm::getX(l1[m.queryIdx].getEndPoint()), lsfm::getY(l1[m.queryIdx].getEndPoint())));
            LineSegment<FT,lsfm::Vec2> line2(lsfm::Vec4<FT>(lsfm::getX(l2[m.trainIdx].getStartPoint()), lsfm::getY(l2[m.trainIdx].getStartPoint()), lsfm::getX(l2[m.trainIdx].getEndPoint()), lsfm::getY(l2[m.trainIdx].getEndPoint())));
...
//        imshow("Detected matches", drawKeyLineMatches<FT,DescriptorMatch<FT>>(cam0img, octave0_1, cam1img, octave0_2, good_matches));
...
//        drawLineMatches( cam0img, octave0_1, cam1img, octave0_2, good_matches, lsd_outImg, Scalar::all( -1 ), Scalar::all( -1 ), lsd_mask,
...
//    }
    ...
    inline std::vector<DMatch> lineMatchingLsdAndCV(cv::Mat lsd_descr0, cv::Mat lsd_descr1, cv::Mat mask = cv::Mat::ones(0, 0, CV_8UC1)){
        if(mask.rows = 0 && mask.cols == 0)
            mask = cv::Mat::ones(lsd_descr0.rows, lsd_descr1.rows, CV_8UC1);
...
//        imshow("Detected matches", drawKeyLineMatches<FT,DescriptorMatch<FT>>(cam0grey, klV0, cam1grey, klV1, good_matches));
        return good_matches;
    }

    template<class LSV>
    inline std::vector<DMatch> lineMatchingLsdAndCV(cv::Mat cam0grey, cv::Mat cam1grey, LSV lines0, LSV lines1, cv::Mat mask = cv::Mat::ones(0, 0, CV_8UC1)){
        if(mask.rows = 0 && mask.cols == 0)
            mask = cv::Mat::ones(lines0.size(), lines1.size(), CV_8UC1);
    ...
    inline std::vector<MATCH> leftRightCheck(std::vector<MATCH> mVec1, std::vector<MATCH> mVec2){
        ...
        for(int i = 0; i < mVec1.size(); ++i){
            if(std::find_if(mVec2.begin(), mVec2.end(), [&](const DMatch &arg) {
                                                       return arg.trainIdx == mVec1[i].queryIdx &&
                                                                arg.queryIdx == mVec1[i].trainIdx; }) != mVec2.end() ) {
    ...
    inline cv::Mat createLBD(cv::Mat cam0grey, std::vector<KeyLine> klV, Ptr<BinaryDescriptor> bd = BinaryDescriptor::createBinaryDescriptor()){
        cv::Mat lsd_descr;
    ...
    /// Overloading
    template<class FT>
    inline cv::Mat createLBD(cv::Mat cam0grey, std::vector<LineSegment<FT>> lineVec, Ptr<BinaryDescriptor> bd = BinaryDescriptor::createBinaryDescriptor()){
