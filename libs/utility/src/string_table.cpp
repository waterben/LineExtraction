...
/// @brief String table utilities implementation.
...
std::vector<std::string> StringTable::col(size_t c) const {
  ...
  size_t s = size();
  for (size_t p = c; p < s; p += cols_) ret.push_back(data_[p]);
  return ret;
}
...
StringTable StringTable::transpose() const {
  ...
  for (size_t r = 0; r < rows_; ++r)
    for (size_t c = 0; c < cols_; ++c) ret(c, r) = this->operator()(r, c);
  return ret;
}
