...
///
/// Supported datasets: windmill, BSDS500, MDB stereo, noise,
/// York Urban, Wireframe.
class TestImages {
 public:
  ...
  /// @brief Get path to a York Urban Line Segment Database image.
  /// @param name Image name (e.g., "P1010001.jpg")
  /// @note Dataset must be downloaded using ./tools/scripts/setup_york_urban.sh
  static std::string yorkUrban(const std::string& name) { return get("YorkUrban/images/" + name); }

  /// @brief Get path to a Wireframe dataset image.
  /// @param name Image name (e.g., "00031546.jpg")
  /// @note Dataset must be downloaded using ./tools/scripts/setup_wireframe.sh
  static std::string wireframe(const std::string& name) { return get("Wireframe/images/" + name); }

  /// @brief Get path to a ground truth CSV file.
  /// @param name CSV name (e.g., "york_urban_gt.csv" or "wireframe_gt.csv")
  /// @note Generated by the dataset setup scripts
  static std::string groundTruthCsv(const std::string& name) { return get("ground_truth/" + name); }
