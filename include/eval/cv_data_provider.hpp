...
struct FileCVDataProvider : public CVDataProvider {
  ...
  FileCVDataProvider(const boost::filesystem::path& p, const std::string& name, bool recursive = true)
      : CVDataProvider(name), pos_(0) {
    parse(p, recursive);
  }
  FileCVDataProvider(const std::vector<boost::filesystem::path>& f, const std::string& name, bool recursive = true)
      : CVDataProvider(name), pos_(0) {
    parse(f, recursive);
  }
  ...
  void parse(const std::vector<boost::filesystem::path>& folders, bool recursive = true) {
    for_each(folders.begin(), folders.end(),
             [&, this](const boost::filesystem::path& f) { this->parse(f, recursive); });
  }
