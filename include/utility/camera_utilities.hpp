...
namespace lsfm {
    ...
    void findBlackCroppingArea(const cv::Mat img, std::vector<int>& border){
        ...
        cv::Mat upper  = img(cv::Rect(img.size().width / MTYPE(4), MTYPE(0),img.size().width / MTYPE(2),img.size().height / MTYPE(2)));
        cv::Mat lower  = img(cv::Rect(img.size().width / 4, img.size().height / 2, img.size().width / 2, img.size().height / 2));
        cv::Mat left  = img(cv::Rect(0, img.size().height / 4, img.size().width / 2,img.size().height / 2));
        cv::Mat right  = img(cv::Rect(img.size().width / 2, img.size().height / 4, img.size().width / 2, img.size().height / 2));
    ...
    int stereoRectification(const std::string intrinsic_filename, const std::string extrinsic_filename, const cv::Size img_size, cv::Mat& P1, cv::Mat& P2, cv::Mat& map11, cv::Mat& map12, cv::Mat& map21, cv::Mat& map22, const double scaleFactor = 1.0){
    ...
    int stereoRectification(const std::string intrinsic_filename, const std::string extrinsic_filename, const std::string preprocessing_filename, cv::Mat& P1, cv::Mat& P2, cv::Mat& map11, cv::Mat& map12, cv::Mat& map21, cv::Mat& map22, cv::Size& S1, cv::Size& S2, const double scaleFactor = 1.0){
        // reading intrinsic parameters
        ...
        return stereoRectification(intrinsic_filename, extrinsic_filename, S1, P1, P2, map11, map12, map21, map22, scaleFactor);
    }

    int stereoRectification(const std::string intrinsic_filename, const std::string extrinsic_filename, const std::string preprocessing_filename, cv::Mat& P1, cv::Mat& P2, cv::Mat& map11, cv::Mat& map12, cv::Mat& map21, cv::Mat& map22, const double scaleFactor = 1.0){
        cv::Size S1(0,0);
        cv::Size S2(0,0);
        return stereoRectification(intrinsic_filename, extrinsic_filename, preprocessing_filename, P1, P2, map11, map12, map21, map22, S1, S2, scaleFactor);
    }
    ...
    public:
        ...
        StereoPreprocessor(const std::string intrinsic_filename, const std::string extrinsic_filename, const std::string preprocessing_filename){
            valid = false;
            ...
            cv::Size S1, S2;
            if (!lsfm::stereoRectification( intrinsic_filename, extrinsic_filename, preprocessing_filename, P1, P2, map11, map12, map21, map22, S1, S2)){
        ...
        cv::Mat preprocessImage(const cv::Mat img, const int id){
          ...
          if (id == 0) {
            ...
            im_gray = img1(cv::Rect(croppingWidth,croppingHeight,img1.size().width-croppingWidth*2,img1.size().height-croppingHeight*2)).clone();   // crop black borders
            return im_gray;
        }
