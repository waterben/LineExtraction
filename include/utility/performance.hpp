...
namespace lsfm {
    ...
    struct PerformanceMeasure {
        PerformanceMeasure(const std::string& sn = std::string(), const std::string& fn = std::string(), double w = 0, double h = 0)
        ...
        PerformanceResult computeResult() const {
            return computeResult(measures);
        }
    ...
    struct PerformanceTaskBase : public TaskBase {
        ...
        static PerformanceMeasure accumulatedMeasure(const PerformanceMeasureVector& measure,
            const std::string sourceName = std::string(), const std::string filterName = std::string());
    };
    ...
    struct PerformanceTaskDefault : public PerformanceTaskBase {
        ...
        virtual void run(TaskData* data, int loops, bool verbose);
    ...
    struct PerformanceTest : public TaskLoader {
        PerformanceTest(const std::string& testName = std::string()) : TaskLoader(testName), showTotal(false), showMean(true), showStdDev(true), showMegaPixel(true) {}
