...
namespace lsfm {
    ...
    struct TaskBase {
        ...
        virtual void run(TaskData* data, int loops, bool verbose) = 0;
        ...
        inline bool rgb() const {
            return (flags & TASK_RGB) != 0;
        }

        inline bool sqr() const {
            return (flags & TASK_SQR) != 0;
        }
    ...
    struct FileDataProvider : public DataProviderBase {
        ...
        FileDataProvider(const boost::filesystem::path& p, const std::string& name, bool recursive = true)
            : DataProviderBase(name), pos_(0) {
            parse(p, recursive);
        }
        FileDataProvider(const std::vector<boost::filesystem::path>& f, const std::string& name, bool recursive = true)
            : DataProviderBase(name), pos_(0) {
            parse(f, recursive);
        }

        void parse(const std::vector<boost::filesystem::path> &folders, bool recursive = true) {
            for_each(folders.begin(), folders.end(), [&, this](const boost::filesystem::path& f) {
                this->parse(f, recursive);
            });
        }

        void parse(const boost::filesystem::path& folder, bool recursive = true);
