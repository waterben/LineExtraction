...
namespace lsfm {
    ...
    cv::Mat applyBorder(cv::Mat& inout, int border, int borderType = cv::BORDER_CONSTANT, const cv::Scalar& color = cv::Scalar());
    cv::Mat applyBorderCopy(const cv::Mat& in, int border, int borderType = cv::BORDER_CONSTANT, const cv::Scalar& color = cv::Scalar());
    ...
    void saveDir(const cv::Mat& data, const std::string& name, const cv::Mat& mask = cv::Mat(), int border = 0) {
        ...
        if (!mask.empty())
            tmp.setTo(cv::Scalar(0, 0, 0), mask == 0);
    ...
    template<class FT>
    void saveFilterResults(const lsfm::FilterResults &results, const std::string &name, bool sqr = false, int border = 0, double th_low = 0, double th_high = 0) {
        ...
        for_each(results.begin(), results.end(), [&](const FilterResult& result) {
            ...
            if (filter == "laplace") {
                //std::cout << "laplace range upper: " << result.second.range.size() << std::endl;
                if (th_high) {
                ...
                if (f->second.data.type() != cv::DataType<FT>::type)
                    tmpx = convertTo<FT>(f->second.data);
                
                f = results.find("gy");
                if (f->second.data.type() != cv::DataType<FT>::type)
                    tmpy = convertTo<FT>(f->second.data);
