...
public:
    ...
    inline void setAxis(double start_x, double end_x, double start_y, double end_y, double scale_x, double scale_y, double border_x, double border_y) {
        PlotWindow* plot = getFigure();
    ...
    inline void setAxis(const std::string& name, double start_x, double end_x, double start_y, double end_y, double scale_x, double scale_y)  {
        PlotWindow* plot = getFigure(name);
        plot->setAxis(start_x, end_x, start_y, end_y, scale_x, scale_y);
    }
    inline void setAxis(const std::string& name, double start_x, double end_x, double start_y, double end_y, double scale_x, double scale_y, double border_x, double border_y)  {
        PlotWindow* plot = getFigure(name);
        plot->setAxis(start_x, end_x, start_y, end_y, scale_x, scale_y, border_x, border_y);
    }
    ...
    template<typename T>
    inline void plot(const std::vector<T>& X, const std::vector<T>& Y, bool fit_axis = false, const QPen pen = QPen(Qt::blue), const QBrush brush = QBrush()) {
        PlotWindow* plot = getFigure();
        plot->plot(X, Y, fit_axis, pen, brush);
    }
    
    inline void plot(const QVector<double>& X, const QVector<double>& Y, bool fit_axis = false, const QPen pen = QPen(Qt::blue), const QBrush brush = QBrush()) {
        PlotWindow* plot = getFigure();
        plot->plot(X, Y, fit_axis, pen, brush);
    }
    ...
    inline void plot(const cv::Mat& mat, bool fit_axis = false, const QCPRange& prange_x = QCPRange(), const QCPRange& prange_y = QCPRange(), const QCPRange& drange = QCPRange(), const QCPColorGradient& grad = QCPColorGradient::gpGrayscale, QCPAxis::ScaleType st = QCPAxis::stLinear, bool interpolate = false) {
        PlotWindow* plot = getFigure();
    ...
    //! plot X vector versus Y vector
    template<typename T>
    inline void plot(const std::string& name,  const std::vector<T>& X, const std::vector<T>& Y, bool fit_axis = false, const QPen pen = QPen(Qt::blue), const QBrush brush = QBrush()) {
        PlotWindow* plot = getFigure(name);
        plot->plot(X, Y, fit_axis, pen, brush);
    }
    
    inline void plot(const std::string& name, const QVector<double>& X, const QVector<double>& Y, bool fit_axis = false, const QPen pen = QPen(Qt::blue), const QBrush brush = QBrush()) {
        PlotWindow* plot = getFigure(name);
        plot->plot(X, Y, fit_axis, pen, brush);
    }
    ...
    //! plot line
    template<typename T>
    inline void line(const std::string& name, T x1, T y1, T x2, T y2, bool fit_axis = false, const QPen pen = QPen(Qt::blue)) {
        PlotWindow* plot = getFigure(name);
    ...
    //! plot line segement
    template<typename T>
    inline void line(const std::string& name, const lsfm::LineSegment<T>& line, bool fit_axis = false, const QPen pen = QPen(Qt::blue)) {
        PlotWindow* plot = getFigure(name);
        plot->line(line, fit_axis, pen);
    }

    //! plot mat as QCPColorMap
    inline void plot(const std::string& name,  const cv::Mat& mat, bool fit_axis = false, const QCPRange& prange_x = QCPRange(), const QCPRange& prange_y = QCPRange(), const QCPRange& drange = QCPRange(), const QCPColorGradient& grad = QCPColorGradient::gpGrayscale, QCPAxis::ScaleType st = QCPAxis::stLinear, bool interpolate = false)  {
        PlotWindow* plot = getFigure(name);
        plot->plot(mat, fit_axis, prange_x, prange_y, drange, grad, st, interpolate);
    }
