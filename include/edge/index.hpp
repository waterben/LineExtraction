...
namespace lsfm {
    ...
    inline bool neg_sign(float a, float b) {
        return std::signbit(a) ^ std::signbit(b);
    }

    inline bool neg_sign(double a, double b) {
        return std::signbit(a) ^ std::signbit(b);
    }
    ...
    template<class PT>
    struct IndexConvert  {
        static inline void toPoint(index_type idx, PT& p, int cols) {
            index2Point(idx, p, cols);
        }

        static inline void toPoint(index_type idx, PT& p, const cv::Mat &tmp) {
            index2Point(idx, p, tmp.cols);
        }
        ...
        static inline void toIndex(const PT& p, index_type &idx, int cols) {
            point2Index(p,idx,cols);
        }

        static inline index_type toIndex(const PT& p, int cols) {
            return point2Index(p, cols);
        }

        static inline void toIndex(const PT& p, index_type &idx, const cv::Mat &tmp) {
            point2Index(p, idx, tmp.cols);
        }

        static inline index_type toIndex(const PT& p, const cv::Mat &tmp) {
            return point2Index(p, tmp.cols);
        }

        template<class COL>
        static inline void toIndex(const PT *beg, const PT *end, index_type *data, const COL &cols) {
            for (; beg != end; ++beg, ++data)
                toIndex(*beg, *data, cols);
        }
        ...
        static inline void convert(const index_type *beg, const index_type *end, PT *data, const cv::Mat& mag, const cv::Mat& dmap = cv::Mat())  {
            for (; beg != end; ++beg, ++data)
                toPoint(*beg, *data, mag);
        }

        static inline void convertDir(const index_type *beg, const index_type *end, PT *data, const cv::Mat& mag, const cv::Mat& dir = cv::Mat()) {
    ...
    struct IndexConvert<index_type> {
        static inline void toPoint(index_type idx, index_type& p, int cols) {
            p = idx;
        }

        static inline void toPoint(index_type idx, index_type& p, const cv::Mat &tmp) {
            p = idx;
        }
        ...
        static inline void toIndex(const index_type& p, index_type &idx, int cols) {
            idx = p;
        }

        static inline index_type toIndex(const index_type& p, int cols) {
            return p;
        }

        static inline void toIndex(const index_type& p, index_type &idx, const cv::Mat &tmp) {
            idx = p;
        }

        static inline index_type toIndex(const index_type& p, const cv::Mat &tmp) {
            return p;
        }
        ...
        static inline void convert(const index_type *beg, const index_type *end, index_type *data, const cv::Mat& mag, const cv::Mat& dmap = cv::Mat()) {
            for (; beg != end; ++beg, ++data)
                toPoint(*beg, *data, mag);
        }

        static inline void convertDir(const index_type *beg, const index_type *end, index_type *data, const cv::Mat& mag, const cv::Mat& dir = cv::Mat()) {
            convert(beg, end, data, mag, dir);
        }
    };
