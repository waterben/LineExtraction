...
namespace lsfm {
    ...
    class PCMatlab : public PhaseCongruency<IT,double,double,double,double>, public PhaseCong{
        ...
        void init() {
            ...
            this->add("grad_minWaveLength", std::bind(&PCMatlab<IT>::minWaveLength, this, std::placeholders::_1), "Minimal wave length.");
            ...
            this->add("grad_deviationGain", std::bind(&PCMatlab<IT>::deviationGain, this, std::placeholders::_1), "deviationGain.");
            this->add("grad_noiseMethod", std::bind(&PCMatlab<IT>::noiseMethod, this, std::placeholders::_1), "Noise method: 0 = none, -1 = dsdf, -2 = sdf, > 0 = threshold.");
            update();
        }
	...
	public:
        ...
        PCMatlab(int nscale = 4, int minWaveLength = 3, double mult = 2.1,
            double sigmaOnf = 0.55, double k = 3, double cutOff = 0.5, int g = 10,
            double deviationGain = 1.5, int noiseMethod = -1, IT it_low = std::numeric_limits<IT>::lowest(), IT it_high = std::numeric_limits<IT>::max())
            : PhaseCongruency<IT,double,double,double,double>(it_low,it_high),
            PhaseCong(nscale, minWaveLength, mult, sigmaOnf), k_(k), cutOff_(cutOff), g_(g), deviationGain_(deviationGain),
            noiseMethod_(noiseMethod) {
            init();
            
        }

        PCMatlab(const ValueManager::NameValueVector &options, img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max())
            : PhaseCongruency<IT, double, double, double, double>(int_lower, int_upper), PhaseCong(4, 3, 2.1, 0.55), k_(3), cutOff_(0.5), g_(10), deviationGain_(1.5),
            noiseMethod_(-1) {
            init();
            value(options);
        }

        PCMatlab(ValueManager::InitializerList options, img_type int_lower = std::numeric_limits<img_type>::lowest(), img_type int_upper = std::numeric_limits<img_type>::max())
            : PhaseCongruency<IT, double, double, double, double>(int_lower, int_upper), PhaseCong(4, 3, 2.1, 0.55), k_(3), cutOff_(0.5), g_(10), deviationGain_(1.5),
            noiseMethod_(-1) {
            init();
            value(options);
        ...
        Value numScales(const Value &ns = Value::NAV()) { if (ns.type()) {nscale_ = ns; update(); } return nscale_;}
        Value minWaveLength(const Value &mw = Value::NAV()) { if (mw.type()) {minW_ = mw; update(); } return minW_;}
        Value mult(const Value &m = Value::NAV()) { if (m.type()) {mult_ = m; update(); } return mult_;}
        Value sigmaOnf(const Value &so = Value::NAV()) { if (so.type()) {sigmaOnf_ = so; update(); } return sigmaOnf_;}
        ...
        DirectionRange directionRange() const {
            return Polar<double, double>::range();
        }
