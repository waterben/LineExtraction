...
| **DetectorProfile** | `detector_profile.hpp/cpp` | Maps 4 intuitive sliders (detail, gap_tolerance, min_length, precision; 0-100%) to concrete parameters for all 9 LSD detectors. Adaptive scaling via `contrast_factor`/`noise_factor`. Factory: `from_image()`, `from_hints()` |
...
| **DetectorProfilePanel** | `detector_profile_panel.{cpp,h,ui}` | Tune detector parameters via 4 percentage sliders + adaptive image factors |
| **ImageAnalyzerPanel** | `image_analyzer_panel.{cpp,h,ui}` | Analyze image properties (contrast, noise, edge density, range) + suggest profile |
...
- Category + image combo box row in the Line Analyzer
...
- `image_analyzer_panel`: Emits `sourcesChanged` from `loadImage()` + grayscale conversion fix
- `continuity_optimizer.ui`: Fixed pre-existing bug (maxHeight=110 < minHeight=156)
