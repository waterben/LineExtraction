...
PlotWindow::PlotWindow(const char* title, QWidget *parent) : QMainWindow(parent) {
    if (objectName().isEmpty())
        setObjectName(QStringLiteral("plotWindow"));
...
void PlotWindow::reset(){
    ...
    horizontalScrollBar->setValue(qRound(qplot->xAxis->range().center() * scale_x)); // adjust position of scroll bar slider
    horizontalScrollBar->setPageStep(qRound(qplot->xAxis->range().size() * scale_x)); // adjust size of scroll bar slider

    verticalScrollBar->setValue(qRound(qplot->yAxis->range().center() * scale_y)); // adjust position of scroll bar slider
...
void PlotWindow::setAxis(double start_x, double end_x, double start_y, double end_y, double scale_x, double scale_y, double border_x, double border_y) {
    ...
    double ysc_start = start_y * scale_y, ysc_end = end_y * scale_y, xsc_start = start_x * scale_x, xsc_end = end_x * scale_x;
...
QCPGraph* PlotWindow::plot(const QVector<double>& X, const QVector<double>& Y, bool fit_axis, const QPen pen, const QBrush brush) {
    if (fit_axis) {
...
QCPColorMap* PlotWindow::plot(const cv::Mat& mat, bool fit_axis, const QCPRange& prange_x, const QCPRange& prange_y, const QCPRange& drange, const QCPColorGradient& grad, QCPAxis::ScaleType st, bool interpolate)
{
