...
1. **Load an image** in the ControlWindow and **click "Load"**. The Detector Profile panel automatically analyzes the image and displays its properties (contrast, noise level, edge density, dynamic range).
2. **Select a detector** from the ControlWindow dropdown.
...
4. **Click "Apply to Detector"** (`pb_apply`). The profile settings are translated into concrete detector parameters and pushed to the active detector. The ControlWindow's parameter table updates to reflect the new values.
5. **Click "Process"** in the ControlWindow to run detection with the new parameters.
...
| File | Purpose |
|------|---------|
| [detectorprofilepanel.h](detectorprofilepanel.h) | Panel class declaration (inherits `LATool`) |
| [detectorprofilepanel.cpp](detectorprofilepanel.cpp) | DetectorProfile + ImageAnalyzer integration, knob-to-parameter mapping |
| [detectorprofilepanel.ui](detectorprofilepanel.ui) | Qt Designer layout |

## Dependencies

- **ControlWindow** — detector access, image source signals
