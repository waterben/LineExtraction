...
LineAnalyser2D::LineAnalyser2D(QWidget* parent)
    : LATool("GT Inspector", parent),
      ...
      lplot(new PlotWindow("GT Inspector Image Viewer", this)),
  ...
  addHelpButton(this, "extensions/ground_truth_inspector/README.md");
}

LineAnalyser2D::~LineAnalyser2D() { delete ui; }

void LineAnalyser2D::connectTools(Analyzer* w) {
  this->ctrl = w;
...
void LineAnalyser2D::loadBundledExample(const std::string& txt_relative,
  ...
  // Load the matching image into the Analyzer first so that srcImg is
...
void LineAnalyser2D::computeCorrectLines() {
  ...
  try {
    ...
    double angle = this->ui->dsb_angle_threshold->value() * CV_PI / 180.0;
 ...
 *          Draws the computed lines which are given from the analyzer
 */
 ...
 *          Loads given image sources from analyzer
 */
 ...
 *          Displays the computed lines from the analyzer
 */
