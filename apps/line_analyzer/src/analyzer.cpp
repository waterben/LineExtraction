...
Analyzer::Analyzer(QWidget* parent)
  ...
  // can quickly apply optimized parameter presets (fast/balanced/accurate).
  {
    ...
    // Insert into the detector params layout (after the label, before the spacer).
    ui->layout_detector_params->insertWidget(1, lblPreset);
    ui->layout_detector_params->insertWidget(2, cbPreset);

    connect(cbPreset, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &Analyzer::applyPreset);
  }

  // --- Detector Profile Button ---
  // Adds a "Profile" button to the detector params row so the user can
  // open the high-level detector tuning panel directly from here.
  {
    ...
    auto* profilePanel = new DetectorProfilePanel(this);
    profilePanel->connectTools(this);
    connect(pbDetectorProfile, &QPushButton::clicked, profilePanel, &QWidget::show);
  }

  // --- Quiver Panel ---
  // Create QuiverPanel and wire it to the Quivers button in the image section.
  {
    auto* quiverPanel = new QuiverPanel(this);
    quiverPanel->connectTools(this);
    connect(ui->pb_quivers, &QPushButton::clicked, quiverPanel, &QWidget::show);
  }
...
void Analyzer::openPreprocess() { pp->show(); }

void Analyzer::setImagePath(const QString& path) {
  ui->le_image_filename->setText(path);
  ui->pb_pre->setEnabled(true);
  ui->pb_load->setEnabled(true);
  loadImage();
...
void Analyzer::updateSource(bool fit_axis) {
...
void Analyzer::updateSourceOptions() {
  if (!imgMap) return;
  const int sourceIdx = ui->cb_image_source->currentIndex();
  if (sourceIdx < 0) return;
  const std::size_t sourceIndex{static_cast<std::size_t>(sourceIdx)};
...
void Analyzer::showQuiverControl() {
  // QuiverPanel is now wired to pb_quivers in the image section.
  // This slot is kept for .ui compatibility but does nothing.
}


void Analyzer::sourceChanged() {
  ui->cb_image_mode->blockSignals(true);
  ui->cb_image_mode->clear();
  if (sources.empty()) {
    ui->cb_image_mode->blockSignals(false);
...
void Analyzer::fitImage() {
  if (sources.empty()) return;
  lplot->setAxis(QCPRange(0, src.cols - 1), QCPRange(0, src.rows - 1));
  lplot->replot();
}

void Analyzer::addDetector(const DetectorPtr& detector) {
  if (!detector) return;
  ...
  if (ui->cb_detector_select->currentIndex() == -1) {
    ...
    if (pbDetectorProfile != nullptr) {
      pbDetectorProfile->setEnabled(true);
    }
  }
  ui->cb_detector_select->addItem(detector->name);
}

void Analyzer::selectDetector() {
...
DetectorPtr Analyzer::getCurrentDetector() const {
  const int detectorIdx = ui->cb_detector_select->currentIndex();
...
QString Analyzer::getCurrentDetectorName() const {
  auto det = getCurrentDetector();
  return det ? det->name : QString();
}

void Analyzer::refreshDetectorOptions() { selectDetector(); }

void Analyzer::resetDetector() {
  const int detectorIdx = ui->cb_detector_select->currentIndex();
  if (detectorIdx < 0) return;
  const std::size_t detectorIndex{static_cast<std::size_t>(detectorIdx)};
  if (detectorIndex >= detectors.size()) return;
...
void Analyzer::resetAllDetectors() {
...
std::string Analyzer::findPresetsPath() {
  namespace fs = std::filesystem;
...
std::pair<bool, lsfm::DetectorId> Analyzer::resolveDetectorId(const QString& name) {
...
void Analyzer::updatePresetCombo() {
...
void Analyzer::applyPreset(int presetIndex) {
...
void Analyzer::setDetectorOption(int row, int col) {
  const int detectorIdx = ui->cb_detector_select->currentIndex();
  if (detectorIdx < 0) return;
  const std::size_t detectorIndex{static_cast<std::size_t>(detectorIdx)};
  if (detectorIndex >= detectors.size()) return;
...
void Analyzer::setLines(const LineSegmentVector& ls) {
  clearLines();
...
void Analyzer::updateLineGeometry(Line& l) {
...
void Analyzer::updateLine(int idx) {
  if (idx < 0) return;
...
void Analyzer::updateLine() { updateLine(lineSel); }

void Analyzer::updateLines() {
  int idx = 0;
...
void Analyzer::updateLineLayers() {
...
void Analyzer::updateLineTableData(int idx) {
  if (idx < 0) return;
  const std::size_t index{static_cast<std::size_t>(idx)};
  if (index >= lines.size()) return;
...
void Analyzer::updateLineByCellChange(int row, int col) {
  if (col > 3) return;
  if (row < 0) return;
...
void Analyzer::clearLineSelected() {
...
void Analyzer::selectLineByRow() { selectLine(ui->table_lines->currentRow()); }

void Analyzer::selectLine(int sel) {
...
void Analyzer::resetAllControls() {
  for_each(lines.begin(), lines.end(), [&](Line& l) { l.reset(); });
  updateLines();
}

void Analyzer::setControls(const LineMod* mod) {
  bool disable;
  LineMod tmp;
  if (mod == nullptr) {
    disable = true;
...
void Analyzer::addTool(LATool* tool) {
...
void Analyzer::setRotSliderValue(double val) {
  ui->slider_line_rot->blockSignals(true);
...
void Analyzer::setRotSpinValue(int val) {
  ui->spin_line_rot->blockSignals(true);
...
void Analyzer::setOTransSliderValue(double val) {
  ui->slider_line_otrans->blockSignals(true);
...
void Analyzer::setOTransSpinValue(int val) {
  ui->spin_line_otrans->blockSignals(true);
...
void Analyzer::setSTransSliderValue(double val) {
  ui->slider_line_strans->blockSignals(true);
...
void Analyzer::setSTransSpinValue(int val) {
  ui->spin_line_strans->blockSignals(true);
...
void Analyzer::setETransSliderValue(double val) {
  ui->slider_line_etrans->blockSignals(true);
...
void Analyzer::setETransSpinValue(int val) {
  ui->spin_line_etrans->blockSignals(true);
...
void Analyzer::setTransPrec(int val) {
  int moveOrthoRange = ui->spin_range_otrans->value();
...
void Analyzer::setRotPrec(int val) {
  int rotRange = ui->spin_range_rot->value();
...
void Analyzer::resetControlOptions() {
...
void Analyzer::setIndicatorVisible(bool val) {
  lplot->qplot->layer("indicator")->setVisible(val);
  lplot->replot();
}

void Analyzer::setIndicatorPosition(double pos, double range) {
  if (lineSel < 0) return;
  const std::size_t index{static_cast<std::size_t>(lineSel)};
  if (index >= lines.size()) return;
...
void Analyzer::setIndicatorPosition(const LineSegment& l, double pos, double range) {
  if (!indicator) return;
...
void Analyzer::replot() { lplot->replot(); }
