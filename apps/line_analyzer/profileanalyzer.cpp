...
ProfileAnalyzer::ProfileAnalyzer(QWidget *parent) : LATool("Profile Analyzer", parent), ui(new Ui::ProfileAnalyzer), 
plot(new PlotWindow("Profile Plot", parent)), sources(0), cw(0), linePos(0)
{
...
void ProfileAnalyzer::createX() {
    ...
    X.clear();
    X.reserve(2 * steps + 1);
    for (int start = -steps; start <= steps; ++start)
...
void create_profile(const cv::Mat_<mat_type>& src, const LineSegment& line, const QVector<double>& X, QVector<double>& profile, QVector<double>& std_dev, float_type param, int idx, bool variance, bool sampled, bool fast) {
    ...
    if (variance) {
        ...
        for_each(X.begin(), X.end(), [&](double x){
            ...
            profile.push_back(getMean<mat_type,lsfm::MeanHelper<float_type,cv::Point_>::func_type_variance>(idx,sampled,fast)(var, src, tmp, param));
        ...
        for_each(X.begin(), X.end(), [&](float_type x){
            tmp.translateOrtho(static_cast<float_type>(x));
            profile.push_back(getMean<mat_type, lsfm::MeanHelper<float_type, cv::Point_>::func_type>(idx,sampled,fast)(src, tmp, param));
            tmp = line;
        });
    }
}
...
void ProfileAnalyzer::createProfile() {
    ...
    switch (src.type()) {
        case CV_8S:
            create_profile(reinterpret_cast<const cv::Mat_<char>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
        case CV_8U:
            create_profile(reinterpret_cast<const cv::Mat_<unsigned char>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
        case CV_16S:
            create_profile(reinterpret_cast<const cv::Mat_<short>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
        case CV_16U:
            create_profile(reinterpret_cast<const cv::Mat_<unsigned short>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
        case CV_32S:
            create_profile(reinterpret_cast<const cv::Mat_<int>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
        case CV_32F:
            create_profile(reinterpret_cast<const cv::Mat_<float>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
        case CV_64F:
            create_profile(reinterpret_cast<const cv::Mat_<double>&>(src), line, X, profile, std_dev, param, ui->cb_interp->currentIndex(), variance, sampled, ui->fast_interp->isChecked());
            break;
    }
}
...
template<class mat_type>
void create_sprofile(const cv::Mat_<mat_type>& src, const LineSegment& line, const QVector<double>& X, QVector<double>& sprofile, float_type pos, int idx) {
...
void ProfileAnalyzer::createSProfile() {
    ...
    switch (src.type()) {
    case CV_8S:
        create_sprofile(reinterpret_cast<const cv::Mat_<char>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    case CV_8U:
        create_sprofile(reinterpret_cast<const cv::Mat_<unsigned char>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    case CV_16S:
        create_sprofile(reinterpret_cast<const cv::Mat_<short>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    case CV_16U:
        create_sprofile(reinterpret_cast<const cv::Mat_<unsigned short>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    case CV_32S:
        create_sprofile(reinterpret_cast<const cv::Mat_<int>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    case CV_32F:
        create_sprofile(reinterpret_cast<const cv::Mat_<float>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    case CV_64F:
        create_sprofile(reinterpret_cast<const cv::Mat_<double>&>(src), line, X, single_profile, pos, ui->cb_interp->currentIndex());
        break;
    }
}
