...
#include "ui_detectorprofilepanel.h"
#include <algorithm/detector_profile.hpp>

/// @brief Tool panel for high-level detector parameter tuning.
///
/// Exposes 4 percentage knobs (detail, gap_tolerance, min_length, precision)
...
class DetectorProfilePanel : public LATool {
 ...
 public slots:
  /// @brief Derive knob values automatically from the current source image.
  void autoFromImage();

  /// @brief Apply the current profile knobs to the active detector.
  void applyToDetector();

  /// @brief Reset all knobs and factors to their default values.
  void resetDefaults();
 ...
 private:
  ...
  ///
  /// @param name Display name from ControlWindow.
  ...
  /// @brief Build a DetectorProfile from the current UI knob values.
