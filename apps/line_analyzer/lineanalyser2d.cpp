...
void LineAnalyser2D::lplotSelChange()
{
    ...
    if(items.size())
    {
        ...
        if(layer->name() == "lines")
        {
            ...
            if(debugMode)
                loadLineInformationIntoTable(lines->at(index));
        }
        else if(layer->name() == "gtlines")
        {
            index = items.front()->property("gtlines").toInt();
            if(debugMode)
                loadLineInformationIntoTable(gtLines.at(index));
            showCLines(index);
        }
        else if(layer->name() == "clines")
        {
            ...
            if(debugMode)
                loadLineInformationIntoTable(cLines.at(index));
            if(analysisMode)
                markTableRow(index);
 ...
 * @param   row
 */
void LineAnalyser2D::showCLines(int row)
{
    this->showCLines(row, 0);
}
...
void LineAnalyser2D::showCLines(int y, int x)
{
    ...
    if(gtDataTemp->isDrawn)
        gtDataTemp->line->setSelected(true);
    ...
    if(ui->cb_gtline_plus_clines->isChecked())
    {
        ...
        for_each(gtDataTemp->clData.begin(), gtDataTemp->clData.end(), [&](CLData &cld)
        {
            cld.line->setVisible(true);
        });
        setLayerVisibility("clines", true);
    }
    if(ui->cb_gt_data->isChecked() || !ui->cb_gtline_plus_clines)
        lplot->replot();
}
 ...
 * @param   row
 */
void LineAnalyser2D::showSelectedCLine(int row)
{
    this->showSelectedCLine(row, 0);
}
 ...
 * @return
 */
bool LineAnalyser2D::checkDistance(const lsfm::LineSegment2d &gtline, const lsfm::LineSegment2d &line, double &distance)
{
...
double LineAnalyser2D::anglediff(const lsfm::LineSegment2d &gtline, const lsfm::LineSegment2d &line)
{
    if(gtline.angle() < 0 && line.angle() < 0)
        return fabs(gtline.angle()-line.angle())*180/CV_PI;

    if(gtline.angle() >= 0 && line.angle() >= 0)
        return fabs(gtline.angle()-line.angle())*180/CV_PI;

    if(gtline.angle() < 0 && line.angle() > 0)
        return fabs((CV_PI+gtline.angle())-line.angle())*180/CV_PI;
 ...
 */
bool LineAnalyser2D::checkLineAngles(const lsfm::LineSegment2d &gtline, const lsfm::LineSegment2d &line, double &angle)
{
    if(!this->ui->cb_check_angle->isChecked())
        return true;

    if(gtline.angle() < 0 && line.angle() < 0)
        return fabs(gtline.angle()-line.angle()) <= angle ? true : false;
 ...
 * @return
 */
bool LineAnalyser2D::checkMiscalculation(const lsfm::LineSegment2d &gtline, const lsfm::LineSegment2d &line, double &error)
{
...
{
    if(ui->cb_all_lines->isChecked())
        setLayerVisibility("lines", false); ui->cb_all_lines->setChecked(false);

    if(ui->cb_correct_lines->isChecked())
        setLayerVisibility("clines", false); ui->cb_correct_lines->setChecked(false);

    if(ui->cb_gt_data->isChecked())
        setLayerVisibility("gtlines", false); ui->cb_gt_data->setChecked(false);
}
 ...
 */
void LineAnalyser2D::openAnalyserOptions()
{
    ao->show();
}
...
{
    if(file_path.empty() || file_path == "None")
        return;
        ...
        for_each(lines->begin(), lines->end(), [&](Line &tl)
        {
            ...
            if(checkDistance(gtline, line, distance))
                if(checkLineAngles(gtline, line, angle))
                    ...
                    {
                        CLData clData(line,
                                      miscalculation(gtline,line),
                                      anglediff(gtline,line),
                                      gtline.length()-line.length(),
                                      fabs(gtline.distance(line.startPoint())),
                                      fabs(gtline.distance(line.endPoint())),
                                      gtl.posGT,
                                      posCL);
...
void LineAnalyser2D::addSourceItems()
{
    ...
    for_each(sources->begin(), sources->end(), [&](const ImageSource& s)
    {
        this->ui->cb_image_source->addItem(s.name);
    });
...
void LineAnalyser2D::addModeItems()
{
    ...
    for_each(s.modes.begin(), s.modes.end(), [&](const ImageMode &m)
    {
        this->ui->cb_image_mode->addItem(m.name);
    });
...
void LineAnalyser2D::enableImageSettings()
{
    ...
    if(!lines->empty() && !ui->cb_all_lines->isEnabled())
        this->ui->cb_all_lines->setDisabled(false);

    if(!gtLines.empty() && !ui->cb_gt_data->isEnabled())
        this->ui->cb_gt_data->setDisabled(false);
}
...
void LineAnalyser2D::displaySource()
{
    ...
    this->setLayerVisibility("image", true);
    // Update UI
    if(!this->ui->cb_image->isChecked())
        this->ui->cb_image->setChecked(true);
    this->ui->cb_image_source->blockSignals(false);
}
 ...
 */
void LineAnalyser2D::displayImage()
{
    manageLayerVisibility(ui->cb_image, "image");
}
 ...
 *          Displays the grid
 */
void LineAnalyser2D::displayGrid()
{
    manageLayerVisibility(ui->cb_grid, "grid");
}
 ...
 */
void LineAnalyser2D::displayAllLines()
{
    manageLayerVisibility(ui->cb_all_lines, "lines");
}
...
{
    if(lockedGTLine == 0 || lockedOtherLine == 0)
        return;
    ...
    CLData tmp(*lockedOtherLine,
               miscalculation(*lockedGTLine, *lockedOtherLine),
               anglediff(*lockedGTLine, *lockedOtherLine),
               lockedGTLine->length() - lockedOtherLine->length(),
               lockedGTLine->distance(lockedOtherLine->startPoint()),
               lockedGTLine->distance(lockedOtherLine->endPoint()),
               -1, -1);
...
void LineAnalyser2D::disableAnalysisSettings()
{
    ...
    if(ui->cb_analysis_old_lines->isChecked())
        ui->cb_analysis_old_lines->setChecked(false);
    manageAnalysisSettings(true);
}
 ...
 */
void LineAnalyser2D::displayOldCLines()
{
    manageLayerVisibility(ui->cb_analysis_old_lines, "oldlines");
}
...
{
    lines = line;
    if(!ui->pb_analysis_save->isEnabled())
        ui->pb_analysis_save->setDisabled(false);
    if(!ui->cb_analysis_old_lines->isEnabled())
        ui->cb_analysis_old_lines->setDisabled(false);
 ...
 * @param   row
 */
void LineAnalyser2D::showCurLineReference(int row)
{
    showCurLineReference(row, 0);
}
 ...
 * @param   row
 */
void LineAnalyser2D::showOldLineReference(int row)
{
    showOldLineReference(row, 0);
}
...
{
    if(clDataTemp)
        clDataTemp->line->setSelected(false);
    manageCLineVisibility(true);
 ...
 */
void LineAnalyser2D::showOptimizer()
{
    po->show();
}
...
{
    if(cLines.empty() || cLinesOld.empty())
        return;
