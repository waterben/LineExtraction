...
file(new QFileDialog(this, tr("Open Image"))), lplot(new PlotWindow("Line Plot", this)), pp(new PreProcessing(this)), 
qo(new Quiver(this)), imgMap(0), lineSel(-1), ui(new Ui::ControlWindow), tools(0)
{
    ...
    QStringList lables;
    lables << "angle" << "distance" << "start" << "end" << "length" << "nx" << "ny" << "center X" << "center Y" << "start X" << "start Y" << "end X" << "end Y";
...
void ControlWindow::openPreprocess() {
    pp->show();
}
...
void ControlWindow::loadImage() {
    ...
    if (detectors.size())
        ui->pb_detector_process->setEnabled(true);
    loadSources();
    lplot->show();
    updateSource();
...
void ControlWindow::loadSources() {
    ...
    for_each(sources.begin(), sources.end(), [&](const ImageSource& s) {
        ui->cb_image_source->addItem(s.name);
    });
    ...
    for_each(ms.begin(), ms.end(), [&](const ImageMode &m) {
        ui->cb_image_mode->addItem(m.name);
    });
...
void ControlWindow::updateSource(bool fit_axis) {
    ...
    lplot->hold(true);
    imgMap = lplot->plot(s.data, fit_axis, QCPRange(), QCPRange(), s.range, m.grad, QCPAxis::stLinear, ui->chb_image_interp->isChecked());
    lplot->hold(false);
}
...
void ControlWindow::updateSourceOptions() {
    ...
    else if (!ui->chb_image_grid->isChecked() && lplot->qplot->layer("grid")->index() < lplot->qplot->layer("image")->index())
...
void ControlWindow::showQuiverControl() {
	qo->show();
}
...
void ControlWindow::processQuiver() {
	...
	for_each(quiver.begin(), quiver.end(), [&](Line &l) {
		lplot->qplot->removeItem(l.line);
	});
	quiver.clear();
	...
	double phase_min = 0.0, phase_max = 0.0, mag_min = 0.0, mag_max = 0.0, 
		gx_max = 0.0, gy_max = 0.0, dir_max = 0.0, dir_min = 0.0, energy_max = 0.0;

	if (qo->data_mode == 0) { gradientQuiver(mag, gx, gy); }
	if (qo->data_mode == 1) { angleQuiver(dir, mag); }
	if (qo->data_mode == 2) { angleQuiver(phase, energy); }
    if (qo->data_mode == 3) { angleQuiver(dir + phase, energy); }
...
void ControlWindow::gradientQuiver(cv::Mat mag, cv::Mat gx, cv::Mat gy) {
	...
	if (mag.empty()) { cout << "No mag for quiver computation available." << endl; return; }
	mag.convertTo(mag, CV_64FC1);
	cv::minMaxIdx(mag, &mag_min, &mag_max);
	cv::resize(mag, mag, cv::Size(), 1.0 / qo->interpolation_pixels, 1.0 / qo->interpolation_pixels, qo->interpolation_mode);

	// gx, gy
	if (qo->data_mode == 0) {
		if (gx.empty() || gy.empty()) { cout << "No gx or gy for quiver computation available." << endl; return; }
		gx.convertTo(gx, CV_64FC1);
		gy.convertTo(gy, CV_64FC1);
		cv::minMaxIdx(gx, NULL, &gx_max);
		cv::minMaxIdx(gy, NULL, &gy_max);
		cv::resize(gx, gx, cv::Size(), 1.0 / qo->interpolation_pixels, 1.0 / qo->interpolation_pixels, qo->interpolation_mode);
		cv::resize(gy, gy, cv::Size(), 1.0 / qo->interpolation_pixels, 1.0 / qo->interpolation_pixels, qo->interpolation_mode);
...
void ControlWindow::angleQuiver(cv::Mat ang, cv::Mat ampl) {
	...
	if (ang.empty() || ampl.empty()) { cout << "No phase or even or odd or energy for quiver computation available." << endl; return; }
	ang.convertTo(ang, CV_64FC1);
	ampl.convertTo(ampl, CV_64FC1);
	cv::minMaxIdx(ang, &ang_min, &ang_max);
	cv::minMaxIdx(ampl, NULL, &ampl_max);
	cv::resize(ang, ang, cv::Size(), 1.0 / qo->interpolation_pixels, 1.0 / qo->interpolation_pixels, qo->interpolation_mode);
	cv::resize(ampl, ampl, cv::Size(), 1.0 / qo->interpolation_pixels, 1.0 / qo->interpolation_pixels, qo->interpolation_mode);
...
void ControlWindow::resetAllDetectors() {
    for_each(detectors.begin(), detectors.end(), [](DetectorPtr d){
        d->create();
    });
    selectDetector();
}
...
void ControlWindow::setDetectorOption(int row, int col) {
    ...
    if (option.type() == lsfm::Value::FLOAT && tmp.type() == lsfm::Value::INT)
        tmp = static_cast<double>(tmp.getInt64());
...
void ControlWindow::processData() {
    ...
    std::cout << "Time for line detection: " << (double(cv::getTickCount()) - stime) * 1000 / cv::getTickFrequency() << "ms" << std::endl;
...
void ControlWindow::setLines(const LineSegmentVector& ls) {
    clearLines();
    for_each(ls.begin(), ls.end(), [&](const LineSegment &line){
...
void ControlWindow::processLineData() {
    ...
    for_each(lines.begin(), lines.end(), [&](Line &tl){
        ...
        if (ui->chb_endpoint_show->isChecked())
            tl.line->setHead(QCPLineEnding(QCPLineEnding::esSpikeArrow, 4, 8));
        tl.line->setPen(pen);
        ...
        cv::Point_<float_type> p1 = line.normalLineDist(0, line.centerPoint()), p2 = line.normalLineDist(ui->spin_normal_length->value(), line.centerPoint());
...
void ControlWindow::updateLineGeometry(Line &l) {
    if (l.line == nullptr || l.normal == nullptr)
        return;
    ...
    p1 = seg.normalLineDist(0, seg.centerPoint()), p2 = seg.normalLineDist(ui->spin_normal_length->value(), seg.centerPoint());
...
void ControlWindow::clearLineSelected() {
    ...
    if (items.size()) for_each(items.begin(), items.end(), [&](decltype(*items.begin())& item) {
...
void ControlWindow::selectLineByRow() {
    selectLine(ui->table_lines->currentRow());
}

void ControlWindow::selectLine(int sel) {
    if (sel >= lines.size() || sel == lineSel)
        return;
    clearLineSelected();
    if (sel > -1) {
        lineSel = sel;
...
void ControlWindow::lplotSelChange() {
    ...
    if (items.size()) {
        ...
        lplot->qplot->blockSignals(true);
        if (items.size() > 1) for_each(items.begin() + 1, items.end(), [&](decltype(*items.begin())& item) {
            item->setSelected(false);
        });
        lplot->qplot->blockSignals(false);
