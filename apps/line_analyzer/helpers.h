...
struct ImageSource {
    ImageSource(const QString& n = QString(), const cv::Mat& d = cv::Mat(), const ImageModes& m = ImageModes(), const QCPRange& r = QCPRange()) :
...
struct Detector {
    ...
    inline LineSegmentVector detect(const cv::Mat& src) {
        if (!lsd)
            return LineSegmentVector();
        lsd->detect(src);
        return lsd->lineSegments();
    }

    inline bool magSqr() const {
        return flags & D_MAG_SQR;
    }

    inline bool colorInput() const {
        return flags & D_COLOR_INPUT;
    }
...
void gradSources(const LSD *lsd, ImageSources &src) {
    src.push_back(ImageSource("dir", lsd->grad().direction(), dirMapModes(), QCPRange(lsd->grad().directionRange().lower, lsd->grad().directionRange().upper)));
    lsfm::FilterResults res = lsd->grad().results();
    for_each(res.begin(), res.end(), [&](const lsfm::FilterResult& r) {
        if (r.first == "dir" || r.first == "mag" || r.first == "gx" || r.first == "gy")
            return;
        if (r.first == "phase")
            src.push_back(ImageSource(r.first.c_str(), r.second.data, dirMapModes(),QCPRange(r.second.range.lower, r.second.range.upper)));
        else
...
void gradEdgeSources(const LSD *lsd, ImageSources &src) {
    src.push_back(ImageSource("dir", lsd->edgeSource().direction(), dirMapModes(), QCPRange(lsd->edgeSource().directionRange().lower, lsd->edgeSource().directionRange().upper)));
...
struct DetectorND : public Detector {
    ...
    ImageSources sources(const cv::Mat& src = cv::Mat()) {
        ...
        lsfm::DerivativeGradient<unsigned char, short, int, float_type, lsfm::SobelDerivative, lsfm::QuadraticMagnitude> grad;
