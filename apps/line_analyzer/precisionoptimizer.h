...
private:
    template<
        class mat_type,
        class search_strategy_type,
        class stop_strategy_type
    >
    inline void optimize(const cv::Mat& mag, double d_lower, double d_upper, double r_lower, double r_upper, double mean_param,
    double derivative_prec, typename lsfm::MeanHelper<double, cv::Point_>::func_type mean_op, search_strategy_type search, stop_strategy_type stop) {
        int idx = 0;
        for_each(lines->begin(), lines->end(), [&](Line& l){
            std::cout << "line: " << idx++;
            float_type d = 0, a = 0;
            lsfm::LineOptimizer<mat_type, search_strategy_type, stop_strategy_type>::optimize(mag, l.modSegment(), d, a,
                d_lower, d_upper, r_lower, r_upper, mean_param, derivative_prec, mean_op, search, stop);
            l.mod.angle += a;
            l.mod.distance += d;
        });
    }
