...
void Quiver::applyChanges() {
	// set
	if (ui->rb_gradient->isChecked()) { data_mode = 0; }
	if (ui->rb_direction->isChecked()) { data_mode = 1; }
	if (ui->rb_phase->isChecked()) { data_mode = 2; }
    if (ui->rb_phase_dir->isChecked()) { data_mode = 3; }
	...
	// display
	if (ui->rb_gradient->isChecked()) { ui->l_qmode->setText(QString("Quiver Mode: gx, gy")); }
	if (ui->rb_direction->isChecked()) { ui->l_qmode->setText(QString("Quiver Mode: dir")); }
	if (ui->rb_phase->isChecked()) { ui->l_qmode->setText(QString("Quiver Mode: phase")); }
    if (ui->rb_phase_dir->isChecked()) { ui->l_qmode->setText(QString("Quiver Mode: ph, dir")); }

	if (ui->chb_threshold->isChecked()) { ui->l_threshold_use->setText(QString("Use Threshold: true")); }
	else { ui->l_threshold_use->setText(QString("Use Threshold: false")); }
	ui->l_threshold_val->setText(QString("Threshold Value: ").append(QString::number(ui->sb_threshold->value())));
	if (ui->chb_scaling->isChecked()) { ui->l_scaling_use->setText(QString("Use Scaling: true")); }
	else { ui->l_scaling_use->setText(QString("Use Scaling: false")); }
	ui->l_scaling_val->setText(QString("Interpolating Pixels: ").append(QString::number(ui->sb_interpolate->value())));
	ui->l_interpolation_mode->setText(QString("Interpolation Mode: ").append(QString::number(ui->cb_interpolate->currentIndex())));
...
void Quiver::selectColor() {
	...
	style_set.append(QString::number(red)); style_set.append(",");
	style_set.append(QString::number(green)); style_set.append(",");
	style_set.append(QString::number(blue)); style_set.append(");");
