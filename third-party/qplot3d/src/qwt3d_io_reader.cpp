...
//! set to data begin
bool extract_info(
    FILE* fp, unsigned int& xmesh, unsigned int& ymesh, double& xmin, double& xmax, double& ymin, double& ymax) {
  char* p;
...
bool check_magic(FILE* fp, const char* val) {
  char* p;
  if ((p = read_field(fp, false)) == 0) return false;
...
bool NativeReader::operator()(Plot3D* plot, QString const& fname) {
  ...
  if (!collectInfo(file, fname, xmesh, ymesh, minx, maxx, miny, maxy)) return false;
