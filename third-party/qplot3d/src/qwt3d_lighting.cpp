...
void Plot3D::enableLighting(bool val) {
  if (lighting_enabled_ == val) return;
...
void Plot3D::disableLighting(bool val) { enableLighting(!val); }

bool Plot3D::lightingEnabled() const { return lighting_enabled_; }
...
void Plot3D::applyLight(unsigned light) {
  ...
  GLfloat lightPos[4] = {static_cast<GLfloat>(lights_[light].shift.x), static_cast<GLfloat>(lights_[light].shift.y),
                         static_cast<GLfloat>(lights_[light].shift.z), 1.0};
