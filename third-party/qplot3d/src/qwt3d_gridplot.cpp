...
#include <qplot3d/qwt3d_enrichment_std.h>
...
void SurfacePlot::createDataG() {
  ...
  if (plotStyle() == Qwt3D::POINTS) {
    ...
    if (userplotstyle_p) createEnrichment(*userplotstyle_p);
    return;
  }
  ...
  if (plotStyle() == FILLEDMESH || plotStyle() == WIREFRAME || plotStyle() == HIDDENLINE) {
    ...
    if (step < actualDataG_->columns() && step < actualDataG_->rows()) {
      ...
      for (i = 0; i < actualDataG_->columns() - step; i += step) glVertex3dv(actualDataG_->vertices[i][0]);
      for (j = 0; j < actualDataG_->rows() - step; j += step) glVertex3dv(actualDataG_->vertices[i][j]);
      for (; i >= 0; i -= step) glVertex3dv(actualDataG_->vertices[i][j]);
      for (; j >= 0; j -= step) glVertex3dv(actualDataG_->vertices[0][j]);
      glEnd();
    }
    ...
    for (i = step; i < actualDataG_->columns() - step; i += step) {
      ...
      for (j = 0; j < actualDataG_->rows(); j += step) glVertex3dv(actualDataG_->vertices[i][j]);
      glEnd();
    }
    for (j = step; j < actualDataG_->rows() - step; j += step) {
      glBegin(GL_LINE_STRIP);
      for (i = 0; i < actualDataG_->columns(); i += step) glVertex3dv(actualDataG_->vertices[i][j]);
      glEnd();
    }
  }
}
...
void SurfacePlot::createNormalsG() {
  if (!normals() || actualDataG_->empty()) return;
...
void SurfacePlot::readIn(GridData& gdata, Triple** data, unsigned int columns, unsigned int rows) {
  ...
  for (unsigned i = 0; i != columns; ++i) {
    for (unsigned j = 0; j != rows; ++j) {
      ...
      if (data[i][j].x > range.maxVertex.x) range.maxVertex.x = data[i][j].x;
      if (data[i][j].y > range.maxVertex.y) range.maxVertex.y = data[i][j].y;
      if (data[i][j].z > range.maxVertex.z) range.maxVertex.z = data[i][j].z;
      if (data[i][j].x < range.minVertex.x) range.minVertex.x = data[i][j].x;
      if (data[i][j].y < range.minVertex.y) range.minVertex.y = data[i][j].y;
      if (data[i][j].z < range.minVertex.z) range.minVertex.z = data[i][j].z;
    }
  }
  gdata.setHull(range);
}
...
void SurfacePlot::readIn(GridData& gdata,
  ...
  for (unsigned i = 0; i != columns; ++i) {
    for (unsigned j = 0; j != rows; ++j) {
      ...
      if (data[i][j] > tmax) tmax = data[i][j];
      if (data[i][j] < tmin) tmin = data[i][j];
    }
  }
...
void SurfacePlot::Isolines2FloorG() {
  if (isolines() <= 0 || actualData_p->empty()) return;
  ...
  for (int k = 0; k != isolines(); ++k) {
    ...
    for (int i = 0; i < cols - step; i += step) {
      for (int j = 0; j < rows - step; j += step) {
        t[0] =
            Triple(actualDataG_->vertices[i][j][0], actualDataG_->vertices[i][j][1], actualDataG_->vertices[i][j][2]);

        col = (*datacolor_p)(t[0].x, t[0].y, t[0].z);
        glColor4d(col.r, col.g, col.b, col.a);
        //  			glColor4d(0,0,0,1);

        t[1] = Triple(actualDataG_->vertices[i + step][j][0], actualDataG_->vertices[i + step][j][1],
                      actualDataG_->vertices[i + step][j][2]);
        t[2] = Triple(actualDataG_->vertices[i + step][j + step][0], actualDataG_->vertices[i + step][j + step][1],
                      actualDataG_->vertices[i + step][j + step][2]);
        t[3] = Triple(actualDataG_->vertices[i][j + step][0], actualDataG_->vertices[i][j + step][1],
        ...
        for (int m = 0; m != 4; ++m) {
          ...
          if ((val >= t[m].z && val <= t[mm].z) || (val >= t[mm].z && val <= t[m].z)) {
            ...
            intersection.push_back(
                Triple(t[m].x + lambda * (t[mm].x - t[m].x), t[m].y + lambda * (t[mm].y - t[m].y), val));
          }
        }
