...
void SurfacePlot::setColorFromVertexC(int node, bool skip) {
  ...
  RGBA col = (*datacolor_p)(actualDataC_->nodes[node].x, actualDataC_->nodes[node].y, actualDataC_->nodes[node].z);
...
void SurfacePlot::Isolines2FloorC() {
  ...
  for (int k = 0; k != isolines(); ++k) {
    ...
    for (unsigned i = 0; i != actualDataC_->cells.size(); ++i) {
      nodes.clear();
      unsigned cellnodes = static_cast<unsigned>(actualDataC_->cells[i].size());
      ...
      for (unsigned m = 0; m != cellnodes; ++m) {
        ...
        if ((val >= nodes[m].z && val <= nodes[mm].z) || (val >= nodes[mm].z && val <= nodes[m].z)) {
          ...
          intersection.push_back(Triple(nodes[m].x + lambda * (nodes[mm].x - nodes[m].x),
                                        nodes[m].y + lambda * (nodes[mm].y - nodes[m].y), val));
        }
      }
...
void SurfacePlot::createNormalsC() {
  if (!normals() || actualData_p->empty()) return;

  if (actualDataC_->nodes.size() != actualDataC_->normals.size()) return;
  Arrow arrow;
...
bool SurfacePlot::loadFromData(TripleField const& data, CellField const& poly) {
  ...
  for (i = 0; i < poly.size(); ++i) {
    if (poly[i].size() < 3)
      ...
      for (unsigned j = 0; j < poly[i].size(); ++j) {
        ...
        unsigned pjj = static_cast<unsigned>((j) ? j - 1 : poly[i].size() - 1);
  ...
  for (i = 0; i != data.size(); ++i) {
    if (data[i].x < hull.minVertex.x) hull.minVertex.x = data[i].x;
    if (data[i].y < hull.minVertex.y) hull.minVertex.y = data[i].y;
    if (data[i].z < hull.minVertex.z) hull.minVertex.z = data[i].z;

    if (data[i].x > hull.maxVertex.x) hull.maxVertex.x = data[i].x;
    if (data[i].y > hull.maxVertex.y) hull.maxVertex.y = data[i].y;
    if (data[i].z > hull.maxVertex.z) hull.maxVertex.z = data[i].z;
  }
