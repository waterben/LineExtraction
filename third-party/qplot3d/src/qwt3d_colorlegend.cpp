...
#include <qplot3d/qwt3d_colorlegend.h>
...
void ColorLegend::setTitleString(QString const& s) { caption_.setString(s); }
...
void ColorLegend::setLimits(double start, double stop) { axis_.setLimits(start, stop); }

void ColorLegend::setMajors(int majors) { axis_.setMajors(majors); }

void ColorLegend::setMinors(int minors) { axis_.setMinors(minors); }

void ColorLegend::setAutoScale(bool val) { axis_.setAutoScale(val); }

void ColorLegend::setScale(SCALETYPE val) { axis_.setScale(val); }

void ColorLegend::setScale(Scale* val) { axis_.setScale(val); }
...
void ColorLegend::setOrientation(ORIENTATION orientation, SCALEPOSITION pos) {
  ...
  if (orientation_ == BottomTop) {
    if (axisposition_ == Bottom || axisposition_ == Top) axisposition_ = Left;
  } else {
    if (axisposition_ == Left || axisposition_ == Right) axisposition_ = Bottom;
  }
}
...
void ColorLegend::draw() {
  ...
  double h = (orientation_ == ColorLegend::BottomTop) ? (two - one).z / colors.size() : (two - one).x / colors.size();
  ...
  unsigned size = static_cast<unsigned>(colors.size());
  RGBA rgb;
