...
/**
    ...
    \see mouseMoveEvent()
    */
void Plot3D::mouseReleaseEvent(QMouseEvent*) { mpressed_ = false; }
...
void Plot3D::setRotationMouse(MouseState bstate, double accel, QPoint diff) {
  ...
  if (bstate == xrot_mstate_) new_xrot = roundi(xRotation() + relyz) % 360;
  if (bstate == yrot_mstate_) new_yrot = roundi(yRotation() + relx) % 360;
  if (bstate == zrot_mstate_) new_zrot = roundi(zRotation() + relx) % 360;
...
void Plot3D::setScaleMouse(MouseState bstate, double accel, QPoint diff) {
  ...
  if (bstate == xscale_mstate_) new_xscale = max(0.0, xScale() + relx);
  if (bstate == yscale_mstate_) new_yscale = max(0.0, yScale() - relyz);
  if (bstate == zscale_mstate_) new_zscale = max(0.0, zScale() - relyz);

  setScale(new_xscale, new_yscale, new_zscale);

  if (bstate == zoom_mstate_) setZoom(max(0.0, zoom() - relyz));
}
...
void Plot3D::setShiftMouse(MouseState bstate, double accel, QPoint diff) {
  ...
  if (bstate == xshift_mstate_) new_xshift = xViewportShift() + relx;
  if (bstate == yshift_mstate_) new_yshift = yViewportShift() - relyz;
...
void Plot3D::setRotationKeyboard(KeyboardState kseq, double speed) {
  ...
  if (kseq == xrot_kstate_[0]) new_xrot = roundi(xRotation() + relyz) % 360;
  if (kseq == xrot_kstate_[1]) new_xrot = roundi(xRotation() - relyz) % 360;
  if (kseq == yrot_kstate_[0]) new_yrot = roundi(yRotation() + relx) % 360;
  if (kseq == yrot_kstate_[1]) new_yrot = roundi(yRotation() - relx) % 360;
  if (kseq == zrot_kstate_[0]) new_zrot = roundi(zRotation() + relx) % 360;
  if (kseq == zrot_kstate_[1]) new_zrot = roundi(zRotation() - relx) % 360;
...
void Plot3D::setScaleKeyboard(KeyboardState kseq, double speed) {
  ...
  if (kseq == xscale_kstate_[0]) new_xscale = max(0.0, xScale() + relx);
  if (kseq == xscale_kstate_[1]) new_xscale = max(0.0, xScale() - relx);
  if (kseq == yscale_kstate_[0]) new_yscale = max(0.0, yScale() - relyz);
  if (kseq == yscale_kstate_[1]) new_yscale = max(0.0, yScale() + relyz);
  if (kseq == zscale_kstate_[0]) new_zscale = max(0.0, zScale() - relyz);
  if (kseq == zscale_kstate_[1]) new_zscale = max(0.0, zScale() + relyz);

  setScale(new_xscale, new_yscale, new_zscale);

  if (kseq == zoom_kstate_[0]) setZoom(max(0.0, zoom() - relyz));
  if (kseq == zoom_kstate_[1]) setZoom(max(0.0, zoom() + relyz));
}
...
void Plot3D::setShiftKeyboard(KeyboardState kseq, double speed) {
  ...
  if (kseq == xshift_kstate_[0]) new_xshift = xViewportShift() + relx;
  if (kseq == xshift_kstate_[1]) new_xshift = xViewportShift() - relx;
  if (kseq == yshift_kstate_[0]) new_yshift = yViewportShift() - relyz;
  if (kseq == yshift_kstate_[1]) new_yshift = yViewportShift() + relyz;
