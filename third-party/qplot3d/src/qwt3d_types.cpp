...
int ccw(coordinate_type** P, int i, int j, int k) {
  coordinate_type a = P[i][0] - P[j][0], b = P[i][1] - P[j][1], c = P[k][0] - P[j][0], d = P[k][1] - P[j][1];
...
QColor Qwt3D::GL2Qt(GLdouble r, GLdouble g, GLdouble b) {
  return QColor(roundi(r * 255), roundi(g * 255), roundi(b * 255));
}
...
unsigned Qwt3D::tesselationSize(CellField const& t) {
  ...
  for (unsigned i = 0; i != t.size(); ++i) ret += static_cast<unsigned>(t[i].size());
