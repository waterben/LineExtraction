...
void SurfacePlot::showNormals(bool b) { datanormals_p = b; }
...
void SurfacePlot::setResolution(int res) {
  if (!actualData_p || actualData_p->datatype == Qwt3D::POLYGON) return;

  if ((resolution_p == res) || res < 1) return;

  resolution_p = res;
  updateNormals();
  updateData();
  if (initializedGL()) updateGL();
...
pair<int, int> SurfacePlot::facets() const {
  if (!hasData()) return pair<int, int>(0, 0);
...
void SurfacePlot::createEnrichment(Enrichment& p) {
  ...
  if (actualData_p->datatype == Qwt3D::POLYGON) {
    for (unsigned i = 0; i != actualDataC_->normals.size(); ++i) ve->draw(actualDataC_->nodes[i]);
  } else if (actualData_p->datatype == Qwt3D::GRID) {
    int step = resolution();
