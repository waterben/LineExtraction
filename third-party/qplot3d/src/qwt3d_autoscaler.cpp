#include <qplot3d/qwt3d_autoscaler.h>
#include <qplot3d/qwt3d_helper.h>
...
\endverbatim
*/
int LinearAutoScaler::segments(
    int& l_intervals, int& r_intervals, double start, double stop, double anchor, double m, int n) {
...
int LinearAutoScaler::execute(double& a, double& b, double start, double stop, int ivals) {
  ...
  if (isPracticallyZero(delta)) return intervals_;
  ...
  while (1) {
    ...
    if (int(c) == 1) {
      ...
      for (unsigned i = static_cast<unsigned>(mantissi_.size() - 1); i > 0; --i) {
