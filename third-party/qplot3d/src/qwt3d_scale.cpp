#include <qplot3d/qwt3d_scale.h>

using namespace Qwt3D;

Scale::Scale() : start_p(0.), stop_p(0.), majorintervals_p(0), minorintervals_p(0), mstart_p(0.), mstop_p(0.) {}
...
void LinearScale::calculate() {
  ...
  if (majorintervals_p) --majorintervals_p;
  ...
  if (isPracticallyZero(step)) return;
...
void LogScale::calculate() {
  ...
  if (start_p < DBL_MIN_10_EXP) start_p = DBL_MIN_10_EXP;
  if (stop_p > DBL_MAX_10_EXP) stop_p = DBL_MAX_10_EXP;
  ...
  while (runningval <= stop_p) {
    if (runningval >= start_p) majors_p.push_back(runningval);
    ++runningval;
  }
  majorintervals_p = static_cast<int>(majors_p.size());
  if (majorintervals_p) --majorintervals_p;
...
void LogScale::setMinors(int val) {
  if ((val == 2) || (val == 3) || (val == 5) || (val == 9)) minorintervals_p = val;
}

LogScale::LogScale() { minorintervals_p = 9; }
