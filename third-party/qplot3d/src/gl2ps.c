...
static int gl2psAllocCompress(unsigned int srcsize) {
  ...
  if (!gl2ps->compress || !srcsize) return GL2PS_ERROR;
...
static void* gl2psReallocCompress(unsigned int srcsize) {
  if (!gl2ps->compress || !srcsize) return NULL;

  if (srcsize < gl2ps->compress->srcLen) return gl2ps->compress->start;
...
static int gl2psDeflate(void) {
  ...
  return compress(gl2ps->compress->dest, &gl2ps->compress->destLen, gl2ps->compress->start, gl2ps->compress->srcLen);
}
...
static void gl2psPrintGzipFooter() {
  ...
  if (gl2ps->options & GL2PS_COMPRESS) {
    if (Z_OK != gl2psDeflate()) {
      ...
      fwrite(gl2ps->compress->dest + n, gl2ps->compress->destLen - (n + 4), 1, gl2ps->stream);
...
static void gl2psListRealloc(GL2PSlist* list, GLint n) {
  ...
  if (!list->array) {
    ...
    if (n > list->nmax) {
      ...
      list->array = (char*)gl2psRealloc(list->array, list->nmax * list->size);
    }
  }
}
...
static void gl2psListRead(GL2PSlist* list, int index, void* data) {
  if ((index < 0) || (index >= list->n)) gl2psMsg(GL2PS_ERROR, "Wrong list index in gl2psListRead");
...
static void gl2psListEncodeBase64(GL2PSlist* list) {
  ...
  while (index < n) {
    ...
    if (len) {
      ...
      for (i = 0; i < 4; i++) gl2psListAdd(list, &out[i]);
    }
  }
  gl2psFree(buffer);
}
...
static GLboolean gl2psSameColor(GL2PSrgba rgba1, GL2PSrgba rgba2) {
  if (!GL2PS_ZERO(rgba1[0] - rgba2[0]) || !GL2PS_ZERO(rgba1[1] - rgba2[1]) || !GL2PS_ZERO(rgba1[2] - rgba2[2]))
    return GL_FALSE;
  return GL_TRUE;
}
...
static GLboolean gl2psSameColorThreshold(int n, GL2PSrgba rgba[], GL2PSrgba threshold) {
  ...
  for (i = 1; i < n; i++) {
    if (fabs(rgba[0][0] - rgba[i][0]) > threshold[0] || fabs(rgba[0][1] - rgba[i][1]) > threshold[1] ||
...
static void gl2psUserWritePNG(png_structp png_ptr, png_bytep data, png_size_t length) {
  ...
  for (i = 0; i < length; i++) gl2psListAdd(png, &data[i]);
}
...
static void gl2psConvertPixmapToPNG(GL2PSimage* pixmap, GL2PSlist* png) {
  ...
  if (!(png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL))) return;
  ...
  png_set_IHDR(png_ptr, info_ptr, pixmap->width, pixmap->height, 8, PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
...
static GLint gl2psAddText(
    GLint type, const char* str, const char* fontname, GLshort fontsize, GLint alignment, GLfloat angle) {
  GLfloat pos[4];
  GL2PSprimitive* prim;
  GLboolean valid;
...
static GLboolean gl2psSupportedBlendMode(GLenum sfactor, GLenum dfactor) {
  ...
  if ((sfactor == GL_SRC_ALPHA && dfactor == GL_ONE_MINUS_SRC_ALPHA) || (sfactor == GL_ONE && dfactor == GL_ZERO))
    return GL_TRUE;
  return GL_FALSE;
}
...
static GLboolean gl2psSamePosition(GL2PSxyz p1, GL2PSxyz p2) {
  if (!GL2PS_ZERO(p1[0] - p2[0]) || !GL2PS_ZERO(p1[1] - p2[1]) || !GL2PS_ZERO(p1[2] - p2[2])) return GL_FALSE;
  return GL_TRUE;
}
...
static GLfloat gl2psPsca(GLfloat* a, GLfloat* b) { return (a[0] * b[0] + a[1] * b[1] + a[2] * b[2]); }
...
static GLfloat gl2psNorm(GLfloat* a) { return (GLfloat)sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]); }
  ...
  switch (prim->type) {
    ...
    case GL2PS_QUADRANGLE:
      ...
      if ((GL2PS_ZERO(v[0]) && GL2PS_ZERO(v[1]) && GL2PS_ZERO(v[2])) ||
        ...
        plane[3] =
            -plane[0] * prim->verts[0].xyz[0] - plane[1] * prim->verts[0].xyz[1] - plane[2] * prim->verts[0].xyz[2];
      }
      break;
    case GL2PS_LINE:
      v[0] = prim->verts[1].xyz[0] - prim->verts[0].xyz[0];
...
static void gl2psCreateSplitPrimitive(GL2PSprimitive* parent,
  ...
  for (i = 0; i < numverts; i++) {
    if (index1[i] < 0) {
      ...
      gl2psCutEdge(&parent->verts[index0[i]], &parent->verts[index1[i]], plane, &child->verts[i]);
    }
  }
}
...
static void gl2psAddIndex(GLshort* index0, GLshort* index1, GLshort* nb, GLshort i, GLshort j) {
  ...
  for (k = 0; k < *nb; k++) {
    if ((index0[k] == i && index1[k] == j) || (index1[k] == i && index0[k] == j)) return;
  }
  index0[*nb] = i;
  index1[*nb] = j;
  (*nb)++;
...
static GLshort gl2psGetIndex(GLshort i, GLshort num) { return (i < num - 1) ? i + 1 : 0; }
...
static void gl2psRescaleAndOffset() {
  ...
  if (!gl2psListNbr(gl2ps->primitives)) return;
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->primitives); i++) {
    ...
    if ((gl2ps->options & GL2PS_SIMPLE_LINE_OFFSET) && (prim->type == GL2PS_LINE)) {
      ...
      area = (prim->verts[1].xyz[0] - prim->verts[0].xyz[0]) * (prim->verts[2].xyz[1] - prim->verts[1].xyz[1]) -
             (prim->verts[2].xyz[0] - prim->verts[1].xyz[0]) * (prim->verts[1].xyz[1] - prim->verts[0].xyz[1]);
      dZdX = ((prim->verts[2].xyz[1] - prim->verts[1].xyz[1]) * (prim->verts[1].xyz[2] - prim->verts[0].xyz[2]) -
              (prim->verts[1].xyz[1] - prim->verts[0].xyz[1]) * (prim->verts[2].xyz[2] - prim->verts[1].xyz[2])) /
             area;
      dZdY = ((prim->verts[1].xyz[0] - prim->verts[0].xyz[0]) * (prim->verts[2].xyz[2] - prim->verts[1].xyz[2]) -
              (prim->verts[2].xyz[0] - prim->verts[1].xyz[0]) * (prim->verts[1].xyz[2] - prim->verts[0].xyz[2])) /
             area;
...
static void gl2psAddPlanesInBspTreeImage(GL2PSprimitive* prim, GL2PSbsptree2d** tree) {
  ...
  if ((*tree == NULL) && (prim->numverts > 2)) {
    ...
    for (i = 0; i < prim->numverts - 1; i++) {
      if (!gl2psGetPlaneFromPoints(prim->verts[i].xyz, prim->verts[i + 1].xyz, head->plane)) {
    ...
    switch (ret) {
      case GL2PS_POINT_INFRONT:
        ...
        for (i = 1 + offset; i < prim->numverts - 1; i++) {
          ...
          if (gl2psGetPlaneFromPoints(prim->verts[i].xyz, prim->verts[i + 1].xyz, cur->front->plane)) {
            cur = cur->front;
            cur->front = NULL;
            cur->back = NULL;
          }
        ...
        if (gl2psGetPlaneFromPoints(prim->verts[i].xyz, prim->verts[offset].xyz, cur->front->plane)) {
      ...
      case GL2PS_POINT_BACK:
        ...
        for (i = 1 + offset; i < prim->numverts - 1; i++) {
          ...
          if (gl2psGetPlaneFromPoints(prim->verts[i + 1].xyz, prim->verts[i].xyz, cur->front->plane)) {
            cur = cur->front;
            cur->front = NULL;
            cur->back = NULL;
          }
        ...
        if (gl2psGetPlaneFromPoints(prim->verts[offset].xyz, prim->verts[i].xyz, cur->front->plane)) {
          cur->front->front = NULL;
          cur->front->back = NULL;
        } else {
          gl2psFree(cur->front);
...
static void gl2psSplitPrimitive2D(GL2PSprimitive* prim,
  ...
  for (i = 0; i <= prim->numverts; i++) {
    ...
    if (((prev == -1) || (prev == cur) || (prev == 0) || (cur == 0)) && (i < prim->numverts)) {
      if (cur == GL2PS_POINT_INFRONT) {
        front_count++;
        front_list = (GL2PSvertex*)gl2psRealloc(front_list, sizeof(GL2PSvertex) * front_count);
        front_list[front_count - 1] = prim->verts[v1];
      } else if (cur == GL2PS_POINT_BACK) {
        back_count++;
        back_list = (GL2PSvertex*)gl2psRealloc(back_list, sizeof(GL2PSvertex) * back_count);
      ...
      front_count++;
      front_list = (GL2PSvertex*)gl2psRealloc(front_list, sizeof(GL2PSvertex) * front_count);
      gl2psCutEdge(&prim->verts[v2], &prim->verts[v1], plane, &front_list[front_count - 1]);
      back_count++;
      back_list = (GL2PSvertex*)gl2psRealloc(back_list, sizeof(GL2PSvertex) * back_count);
...
static GLint gl2psAddInBspImageTree(GL2PSprimitive* prim, GL2PSbsptree2d** tree) {
  ...
  if (prim->type == GL2PS_PIXMAP || prim->type == GL2PS_TEXT || prim->type == GL2PS_SPECIAL) {
    return 1;
  }
  ...
  while (used > 0) {
    ...
    switch ((GLint)*current) {
      case GL_POINT_TOKEN:
        ...
        current += i;
        used -= i;
        gl2psAddPolyPrimitive(GL2PS_POINT, 1, vertices, 0, pattern, factor, psize, 0);
        break;
      case GL_LINE_TOKEN:
      case GL_LINE_RESET_TOKEN:
        ...
        current += i;
        used -= i;
        gl2psAddPolyPrimitive(GL2PS_LINE, 2, vertices, 0, pattern, factor, lwidth, 0);
        break;
        ...
        while (count > 0 && used > 0) {
          ...
          if (v == 2) {
            if (GL_TRUE == boundary) {
              ...
              flag = 0;
            gl2psAddPolyPrimitive(GL2PS_TRIANGLE, 3, vertices, offset, pattern, factor, 1, flag);
      ...
      case GL_PASS_THROUGH_TOKEN:
        switch ((GLint)current[1]) {
          ...
          case GL2PS_IMAGEMAP_TOKEN:
            ...
            v = 2 * sizeoffloat;
            vtot = node->image->height + node->image->height * ((node->image->width - 1) / 8);
          ...
          case GL2PS_TEXT_TOKEN:
            if (auxindex < gl2psListNbr(gl2ps->auxprimitives))
              gl2psListAdd(gl2ps->primitives, gl2psListPointer(gl2ps->auxprimitives, auxindex++));
            else
...
static void gl2psPrintPostScriptImagemap(
    GLfloat x, GLfloat y, GLsizei width, GLsizei height, const unsigned char* imagemap) {
  int i, size;
...
static void gl2psPrintPostScriptHeader(void) {
  ...
  gl2psPrintf(
      ...
      "%%%%Pages: 1\n",
      gl2ps->title, GL2PS_MAJOR_VERSION, GL2PS_MINOR_VERSION, GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION, GL2PS_COPYRIGHT,
  ...
  gl2psPrintf(
      ...
      (gl2ps->options & GL2PS_NO_PS3_SHADING) ? "false" : "true", gl2ps->threshold[0], gl2ps->threshold[1],
      gl2ps->threshold[2]);
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    gl2psPrintf(
        ...
        "closepath fill\n",
        gl2ps->bgcolor[0], gl2ps->bgcolor[1], gl2ps->bgcolor[2], (int)gl2ps->viewport[0], (int)gl2ps->viewport[1],
        (int)gl2ps->viewport[2], (int)gl2ps->viewport[1], (int)gl2ps->viewport[2], (int)gl2ps->viewport[3],
...
static void gl2psResetPostScriptColor(void) { gl2ps->lastrgba[0] = gl2ps->lastrgba[1] = gl2ps->lastrgba[2] = -1.; }

static void gl2psEndPostScriptLine(void) {
  int i;
  if (gl2ps->lastvertex.rgba[0] >= 0.) {
    gl2psPrintf("%g %g LE\n", gl2ps->lastvertex.xyz[0], gl2ps->lastvertex.xyz[1]);
    for (i = 0; i < 3; i++) gl2ps->lastvertex.xyz[i] = -1.;
    for (i = 0; i < 4; i++) gl2ps->lastvertex.rgba[i] = -1.;
  }
}
  ...
  switch (prim->type) {
    ...
    case GL2PS_LINE:
      if (!gl2psSamePosition(gl2ps->lastvertex.xyz, prim->verts[0].xyz) ||
          !gl2psSameColor(gl2ps->lastrgba, prim->verts[0].rgba) || gl2ps->lastlinewidth != prim->width ||
          gl2ps->lastpattern != prim->pattern || gl2ps->lastfactor != prim->factor) {
      ...
      gl2psPrintPostScriptColor(prim->verts[0].rgba);
      gl2psPrintf("%g %g %s\n", prim->verts[0].xyz[0], prim->verts[0].xyz[1], newline ? "LS" : "L");
    ...
    case GL2PS_TRIANGLE:
      if (!gl2psVertsSameColor(prim)) {
        ...
        gl2psPrintf("%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g ST\n", prim->verts[2].xyz[0], prim->verts[2].xyz[1],
                    prim->verts[2].rgba[0], prim->verts[2].rgba[1], prim->verts[2].rgba[2], prim->verts[1].xyz[0],
                    prim->verts[1].xyz[1], prim->verts[1].rgba[0], prim->verts[1].rgba[1], prim->verts[1].rgba[2],
                    prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->verts[0].rgba[0], prim->verts[0].rgba[1],
                    prim->verts[0].rgba[2]);
      } else {
        gl2psPrintPostScriptColor(prim->verts[0].rgba);
        gl2psPrintf("%g %g %g %g %g %g T\n", prim->verts[2].xyz[0], prim->verts[2].xyz[1], prim->verts[1].xyz[0],
                    prim->verts[1].xyz[1], prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
      }
      break;
    case GL2PS_QUADRANGLE:
      ...
      break;
    case GL2PS_PIXMAP:
      gl2psPrintPostScriptPixmap(prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->data.image);
      break;
    case GL2PS_IMAGEMAP:
      if (prim->data.image->type != GL2PS_IMAGEMAP_WRITTEN) {
        gl2psPrintPostScriptColor(prim->verts[0].rgba);
        gl2psPrintPostScriptImagemap(prim->data.image->pixels[0], prim->data.image->pixels[1], prim->data.image->width,
                                     prim->data.image->height, (const unsigned char*)(&(prim->data.image->pixels[2])));
    ...
    case GL2PS_TEXT:
      ...
      if (prim->data.text->angle) gl2psPrintf("%g ", prim->data.text->angle);
      gl2psPrintf("%g %g %d /%s ", prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->data.text->fontsize,
    ...
    case GL2PS_SPECIAL:
      /* alignment contains the format for which the special output text
         is intended */
      if (prim->data.text->alignment == GL2PS_PS || prim->data.text->alignment == GL2PS_EPS)
        gl2psPrintf("%s\n", prim->data.text->str);
      break;
    default:
      break;
...
static void gl2psPrintPostScriptBeginViewport(GLint viewport[4]) {
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    ...
    gl2psPrintf(
        ...
        "newpath %d %d moveto %d %d lineto %d %d lineto %d %d lineto\n"
        "closepath fill\n",
        rgba[0], rgba[1], rgba[2], x, y, x + w, y, x + w, y + h, x, y + h);
  }
...
static GL2PSbackend gl2psPS = {gl2psPrintPostScriptHeader,
...
static GL2PSbackend gl2psEPS = {gl2psPrintPostScriptHeader,
                                gl2psPrintPostScriptFooter,
                                gl2psPrintPostScriptBeginViewport,
                                gl2psPrintPostScriptEndViewport,
                                gl2psPrintPostScriptPrimitive,
...
static void gl2psPrintTeXHeader(void) {
  ...
  fprintf(gl2ps->stream,
          ...
          "%% For: %s\n"
          "%% CreationDate: %s",
          gl2ps->title, GL2PS_MAJOR_VERSION, GL2PS_MINOR_VERSION, GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION,
          GL2PS_COPYRIGHT, gl2ps->producer, ctime(&now));
  ...
  fprintf(gl2ps->stream,
          ...
          name, (gl2ps->options & GL2PS_LANDSCAPE) ? "\\rotatebox{90}{" : "", (int)gl2ps->viewport[2],
          (int)gl2ps->viewport[3]);
}
...
static void gl2psPrintTeXPrimitive(void* data) {
  ...
  switch (prim->type) {
    case GL2PS_TEXT:
      fprintf(gl2ps->stream, "\\fontsize{%d}{0}\n\\selectfont", prim->data.text->fontsize);
      ...
      if (prim->data.text->angle) fprintf(gl2ps->stream, "\\rotatebox{%g}{", prim->data.text->angle);
      fprintf(gl2ps->stream, "\\textcolor[rgb]{%g,%g,%g}{{%s}}", prim->verts[0].rgba[0], prim->verts[0].rgba[1],
              prim->verts[0].rgba[2], prim->data.text->str);
      if (prim->data.text->angle) fprintf(gl2ps->stream, "}");
      fprintf(gl2ps->stream, "}}\n");
      break;
    case GL2PS_SPECIAL:
      /* alignment contains the format for which the special output text
         is intended */
      if (prim->data.text->alignment == GL2PS_TEX) fprintf(gl2ps->stream, "%s\n", prim->data.text->str);
      break;
    default:
      break;
  }
...
static GLint gl2psPrintTeXEndViewport(void) { return gl2psPrintPrimitives(); }

static void gl2psPrintTeXFinalPrimitive(void) {}

/* definition of the LaTeX backend */

static GL2PSbackend gl2psTEX = {gl2psPrintTeXHeader,
                                gl2psPrintTeXFooter,
...
static void gl2psPDFgroupObjectInit(GL2PSpdfgroup* gro) {
  ...
  gro->ptrlist = NULL;
  gro->fontno = gro->gsno = gro->imno = gro->maskshno = gro->shno = gro->trgroupno = gro->fontobjno = gro->imobjno =
      gro->shobjno = gro->maskshobjno = gro->gsobjno = gro->trgroupobjno = -1;
}
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfprimlist); ++i) {
    ...
    switch (p->type) {
      ...
      case GL2PS_TEXT:
        ...
        break;
      case GL2PS_LINE:
        if (lasttype != p->type || lastwidth != p->width || lastpattern != p->pattern || lastfactor != p->factor ||
        ...
        break;
      case GL2PS_POINT:
        if (lasttype != p->type || lastwidth != p->width || !gl2psSameColor(p->verts[0].rgba, lastrgba)) {
          gl2psPDFgroupObjectInit(&gro);
          gro.ptrlist = gl2psListCreate(1, 2, sizeof(GL2PSprimitive*));
          gl2psListAdd(gro.ptrlist, &p);
          gl2psListAdd(gl2ps->pdfgrouplist, &gro);
      ...
      case GL2PS_TRIANGLE:
        ...
        lastTriangleWasNotSimpleWithSameColor = !(tmpt.prop & T_CONST_COLOR && tmpt.prop & T_ALPHA_1) ||
                                                !gl2psSameColor(tmpt.vertex[0].rgba, lastt.vertex[0].rgba);
        if (lasttype == p->type && tmpt.prop == lastt.prop && lastTriangleWasNotSimpleWithSameColor) {
...
static void gl2psSortOutTrianglePDFgroup(GL2PSpdfgroup* gro) {
  ...
  if (!gl2psListNbr(gro->ptrlist)) return;
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i) {
    ...
    switch (prim->type) {
        ...
        if (t.prop & T_CONST_COLOR && t.prop & T_ALPHA_1) {
          ...
          for (j = 0; j <= lastel; ++j) {
            ...
            gl2ps->streamlength += gl2psPrintf(
                ...
                "%f %f l\n"
                "h f\n",
                t.vertex[0].xyz[0], t.vertex[0].xyz[1], t.vertex[1].xyz[0], t.vertex[1].xyz[1], t.vertex[2].xyz[0],
                t.vertex[2].xyz[1]);
          }
        }
      ...
      case GL2PS_PIXMAP:
        for (j = 0; j <= lastel; ++j) {
          prim = *(GL2PSprimitive**)gl2psListPointer(gro->ptrlist, j);
          gl2psPutPDFImage(prim->data.image, gro->imno, prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
        }
        break;
      case GL2PS_TEXT:
        for (j = 0; j <= lastel; ++j) {
          prim = *(GL2PSprimitive**)gl2psListPointer(gro->ptrlist, j);
          gl2ps->streamlength += gl2psPrintPDFFillColor(prim->verts[0].rgba);
          gl2psPutPDFText(prim->data.text, gro->fontno, prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
        }
        break;
      default:
        break;
...
static int gl2psPDFgroupListWriteGStateResources(void) {
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i) {
    gro = (GL2PSpdfgroup*)gl2psListPointer(gl2ps->pdfgrouplist, i);
    if (gro->gsno >= 0) offs += fprintf(gl2ps->stream, "/GS%d %d 0 R\n", gro->gsno, gro->gsobjno);
  }
  offs += fprintf(gl2ps->stream, ">>\n");
  return offs;
}
...
static int gl2psPDFgroupListWriteShaderResources(void) {
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i) {
    gro = (GL2PSpdfgroup*)gl2psListPointer(gl2ps->pdfgrouplist, i);
    if (gro->shno >= 0) offs += fprintf(gl2ps->stream, "/Sh%d %d 0 R\n", gro->shno, gro->shobjno);
    if (gro->maskshno >= 0) offs += fprintf(gl2ps->stream, "/TrSh%d %d 0 R\n", gro->maskshno, gro->maskshobjno);
  }
  offs += fprintf(gl2ps->stream, ">>\n");
  return offs;
}
...
static int gl2psPDFgroupListWriteXObjectResources(void) {
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i) {
    gro = (GL2PSpdfgroup*)gl2psListPointer(gl2ps->pdfgrouplist, i);
    if (!gl2psListNbr(gro->ptrlist)) continue;
    ...
    switch (p->type) {
      ...
      case GL2PS_TRIANGLE:
        if (gro->trgroupno >= 0) offs += fprintf(gl2ps->stream, "/TrG%d %d 0 R\n", gro->trgroupno, gro->trgroupobjno);
        break;
      default:
        break;
    }
...
static int gl2psPrintPDFInfo(void) {
  ...
  offs = fprintf(gl2ps->stream,
                 ...
                 "/Producer (%s)\n",
                 gl2ps->title, GL2PS_MAJOR_VERSION, GL2PS_MINOR_VERSION, GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION,
                 GL2PS_COPYRIGHT, gl2ps->producer);
...
static int gl2psPrintPDFShaderStreamData(GL2PStriangle* triangle,
  ...
  if (gray && gray != 8 && gray != 16) gray = 8;

  for (i = 0; i < 3; ++i) {
    offs += gl2psPrintPDFShaderStreamDataCoord(&triangle->vertex[i], action, dx, dy, xmin, ymin);
    if (gray) {
      v = triangle->vertex[i];
...
static void gl2psPDFRectHull(
    GLfloat* xmin, GLfloat* xmax, GLfloat* ymin, GLfloat* ymax, GL2PStriangle* triangles, int cnt) {
  int i, j;
  ...
  *ymax = triangles[0].vertex[0].xyz[1];
  ...
  for (i = 0; i < cnt; ++i) {
    for (j = 0; j < 3; ++j) {
      if (*xmin > triangles[i].vertex[j].xyz[0]) *xmin = triangles[i].vertex[j].xyz[0];
      if (*xmax < triangles[i].vertex[j].xyz[0]) *xmax = triangles[i].vertex[j].xyz[0];
      if (*ymin > triangles[i].vertex[j].xyz[1]) *ymin = triangles[i].vertex[j].xyz[1];
      if (*ymax < triangles[i].vertex[j].xyz[1]) *ymax = triangles[i].vertex[j].xyz[1];
    }
  }
}
...
static int gl2psPrintPDFShader(int obj, GL2PStriangle* triangles, int size, int gray) {
  ...
  offs += fprintf(gl2ps->stream,
                  ...
                  obj, (gray) ? "/DeviceGray" : "/DeviceRGB", (gray) ? gray : 8, xmin, xmax, ymin, ymax,
                  (gray) ? "" : "0 1 0 1");
  ...
  if (gl2ps->options & GL2PS_COMPRESS) {
    ...
    for (i = 0; i < size; ++i)
      gl2psPrintPDFShaderStreamData(&triangles[i], xmax - xmin, ymax - ymin, xmin, ymin, gl2psWriteBigEndianCompress,
                                    gray);
    ...
    if (Z_OK == gl2psDeflate() && 23 + gl2ps->compress->destLen < gl2ps->compress->srcLen) {
      ...
      offs += gl2ps->compress->destLen * fwrite(gl2ps->compress->dest, gl2ps->compress->destLen, 1, gl2ps->stream);
      done = 1;
    }
    gl2psFreeCompress();
  }
  ...
  if (!done) {
    ...
    for (i = 0; i < size; ++i)
      offs +=
          gl2psPrintPDFShaderStreamData(&triangles[i], xmax - xmin, ymax - ymin, xmin, ymin, gl2psWriteBigEndian, gray);
  }
...
static int gl2psPrintPDFShaderMask(int obj, int childobj) {
  ...
  offs +=
      fprintf(gl2ps->stream,
              ...
              ">>\n",
              obj, (int)gl2ps->viewport[0], (int)gl2ps->viewport[1], (int)gl2ps->viewport[2], (int)gl2ps->viewport[3]);

  len = (childobj > 0) ? strlen("/TrSh sh\n") + (int)log10((double)childobj) + 1 : strlen("/TrSh0 sh\n");
...
static int gl2psPrintPDFShaderExtGS(int obj, int childobj) {
  ...
  offs += fprintf(gl2ps->stream, "/SMask << /S /Alpha /G %d 0 R >> ", childobj);
...
static int gl2psPrintPDFPixmap(int obj, int childobj, GL2PSimage* im, int gray) {
  ...
  offs += fprintf(gl2ps->stream,
                  ...
                  "/ColorSpace %s \n"
                  "/BitsPerComponent 8\n",
                  obj, (int)im->width, (int)im->height, (gray) ? "/DeviceGray" : "/DeviceRGB");
  if (GL_RGBA == im->format && gray == 0) {
    offs += fprintf(gl2ps->stream, "/SMask %d 0 R\n", childobj);
  }
...
static int gl2psPDFgroupListWriteObjects(int entryoffs) {
  ...
  if (!gl2ps->pdfgrouplist) return offs;
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i) {
    ...
    switch (p->type) {
      ...
      case GL2PS_SPECIAL:
        /* alignment contains the format for which the special output text
           is intended */
        if (p->data.text->alignment == GL2PS_PDF) offs += fprintf(gl2ps->stream, "%s\n", p->data.text->str);
        break;
      default:
        break;
    }
...
static void gl2psPrintPDFFooter(void) {
  ...
  gl2ps->xreflist = (int*)gl2psRealloc(gl2ps->xreflist, sizeof(int) * (gl2ps->objects_stack + 1));
  gl2ps->xreflist[7] = offs;

  offs += gl2psPrintPDFGSObject();
  gl2ps->xreflist[8] = offs;

  gl2ps->xreflist[gl2ps->objects_stack] = gl2psPDFgroupListWriteObjects(gl2ps->xreflist[8]);
  ...
  for (i = 1; i < gl2ps->objects_stack; ++i) fprintf(gl2ps->stream, "%010d 00000 n \n", gl2ps->xreflist[i]);
...
static GL2PSbackend gl2psPDF = {gl2psPrintPDFHeader,
                                gl2psPrintPDFFooter,
...
static void gl2psSVGGetCoordsAndColors(int n, GL2PSvertex* verts, GL2PSxyz* xyz, GL2PSrgba* rgba) {
  ...
  for (i = 0; i < n; i++) {
    ...
    xyz[i][2] = 0.0F;
    for (j = 0; j < 4; j++) rgba[i][j] = verts[i].rgba[j];
  }
}
...
static void gl2psPrintSVGHeader(void) {
  ...
  gl2psPrintf(
      ...
      "For: %s\n"
      "CreationDate: %s",
      GL2PS_MAJOR_VERSION, GL2PS_MINOR_VERSION, GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION, GL2PS_COPYRIGHT,
      gl2ps->producer, ctime(&now));
  gl2psPrintf("</desc>\n");
  gl2psPrintf("<defs>\n");
  gl2psPrintf("</defs>\n");
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    ...
    gl2psPrintf("<polygon fill=\"%s\" points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n", col, (int)gl2ps->viewport[0],
                (int)gl2ps->viewport[1], (int)gl2ps->viewport[2], (int)gl2ps->viewport[1], (int)gl2ps->viewport[2],
                (int)gl2ps->viewport[3], (int)gl2ps->viewport[0], (int)gl2ps->viewport[3]);
  }
...
static void gl2psEndSVGLine(void) {
  int i;
  if (gl2ps->lastvertex.rgba[0] >= 0.) {
    gl2psPrintf("%g,%g\"/>\n", gl2ps->lastvertex.xyz[0], gl2ps->viewport[3] - gl2ps->lastvertex.xyz[1]);
    for (i = 0; i < 3; i++) gl2ps->lastvertex.xyz[i] = -1.;
    for (i = 0; i < 4; i++) gl2ps->lastvertex.rgba[i] = -1.;
  }
}
...
static void gl2psPrintSVGPixmap(GLfloat x, GLfloat y, GL2PSimage* pixmap) {
  ...
  png = gl2psListCreate(pixmap->width * pixmap->height * 3, 1000, sizeof(unsigned char));
  gl2psConvertPixmapToPNG(pixmap, png);
  gl2psListEncodeBase64(png);
  gl2psPrintf("<image x=\"%g\" y=\"%g\" width=\"%d\" height=\"%d\"\n", x, y - pixmap->height, pixmap->width,
              pixmap->height);
  ...
  gl2psListDelete(png);
#else
  gl2psMsg(GL2PS_WARNING,
           "GL2PS has to be compiled with PNG support in "
...
static void gl2psPrintSVGPrimitive(void* data) {
  ...
  switch (prim->type) {
    ...
    case GL2PS_LINE:
      ...
      if (newline) {
        gl2psSVGGetColorString(rgba[0], col);
        gl2psPrintf("<polyline fill=\"none\" stroke=\"%s\" stroke-width=\"%g\" ", col, prim->width);
    ...
    case GL2PS_TEXT:
      ...
      gl2psPrintf(
          ...
          col, xyz[0][0], xyz[0][1], prim->data.text->fontsize, prim->data.text->fontname, prim->data.text->str);
      break;
    case GL2PS_SPECIAL:
      /* alignment contains the format for which the special output text
         is intended */
      if (prim->data.text->alignment == GL2PS_SVG) gl2psPrintf("%s\n", prim->data.text->str);
      break;
    default:
      break;
  }
...
static void gl2psPrintSVGBeginViewport(GLint viewport[4]) {
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    ...
    gl2psPrintf("<polygon fill=\"%s\" points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n", col, x, gl2ps->viewport[3] - y, x + w,
                gl2ps->viewport[3] - y, x + w, gl2ps->viewport[3] - (y + h), x, gl2ps->viewport[3] - (y + h));
  }

  gl2psPrintf("<clipPath id=\"cp%d%d%d%d\">\n", x, y, w, h);
  gl2psPrintf("  <polygon points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n", x, gl2ps->viewport[3] - y, x + w,
              gl2ps->viewport[3] - y, x + w, gl2ps->viewport[3] - (y + h), x, gl2ps->viewport[3] - (y + h));
...
static GL2PSbackend gl2psSVG = {gl2psPrintSVGHeader,
                                gl2psPrintSVGFooter,
...
static void gl2psPrintPGFDash(GLushort pattern, GLint factor) {
  ...
  if (pattern == gl2ps->lastpattern && factor == gl2ps->lastfactor) return;
  ...
  switch (prim->type) {
    case GL2PS_POINT:
      ...
      fprintf(gl2ps->stream,
              ...
              "{\\pgfpoint{%fpt}{%fpt}}\n\\pgfusepath{fill}\n",
              prim->verts[0].xyz[0] - 0.5 * prim->width, prim->verts[0].xyz[1] - 0.5 * prim->width, prim->width,
              prim->width);
      break;
    case GL2PS_LINE:
      gl2psPrintPGFColor(prim->verts[0].rgba);
      ...
      fprintf(gl2ps->stream,
              ...
              "\\pgfusepath{stroke}\n",
              prim->verts[1].xyz[0], prim->verts[1].xyz[1], prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
      break;
    case GL2PS_TRIANGLE:
      ...
      fprintf(gl2ps->stream,
              ...
              prim->verts[2].xyz[0], prim->verts[2].xyz[1], prim->verts[1].xyz[0], prim->verts[1].xyz[1],
              prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
      break;
    case GL2PS_TEXT:
      ...
      if (prim->data.text->angle) fprintf(gl2ps->stream, "\\pgftransformrotate{%f}{", prim->data.text->angle);

      fprintf(gl2ps->stream, "\\pgfnode{rectangle}{%s}{\\fontsize{%d}{0}\\selectfont",
              gl2psPGFTextAlignment(prim->data.text->alignment), prim->data.text->fontsize);
    ...
    case GL2PS_SPECIAL:
      /* alignment contains the format for which the special output text
         is intended */
      if (prim->data.text->alignment == GL2PS_PGF) fprintf(gl2ps->stream, "%s\n", prim->data.text->str);
      break;
    default:
      break;
  }
...
static GL2PSbackend gl2psPGF = {gl2psPrintPGFHeader,
                                gl2psPrintPGFFooter,
...
static void gl2psComputeTightBoundingBox(void* data) {
  ...
  for (i = 0; i < prim->numverts; i++) {
    if (prim->verts[i].xyz[0] < gl2ps->viewport[0]) gl2ps->viewport[0] = (GLint)prim->verts[i].xyz[0];
    if (prim->verts[i].xyz[0] > gl2ps->viewport[2]) gl2ps->viewport[2] = (GLint)(prim->verts[i].xyz[0] + 0.5F);
    if (prim->verts[i].xyz[1] < gl2ps->viewport[1]) gl2ps->viewport[1] = (GLint)prim->verts[i].xyz[1];
    if (prim->verts[i].xyz[1] > gl2ps->viewport[3]) gl2ps->viewport[3] = (GLint)(prim->verts[i].xyz[1] + 0.5F);
  }
}
...
static GLint gl2psPrintPrimitives(void) {
  ...
  if (used > 0) gl2psParseFeedbackBuffer(used);

  gl2psRescaleAndOffset();

  if (gl2ps->header) {
    if (gl2psListNbr(gl2ps->primitives) && (gl2ps->options & GL2PS_TIGHT_BOUNDING_BOX)) {
  ...
  switch (gl2ps->sort) {
    ...
    case GL2PS_BSP_SORT:
      ...
      if (gl2ps->options & GL2PS_OCCLUSION_CULL) {
        gl2psTraverseBspTree(root, eye, -GL2PS_EPSILON, gl2psLess, gl2psAddInImageTree, 1);
        gl2psFreeBspImageTree(&gl2ps->imagetree);
      }
      gl2psTraverseBspTree(root, eye, GL2PS_EPSILON, gl2psGreater, gl2psbackends[gl2ps->format]->printPrimitive, 0);
...
GL2PSDLL_API GLint gl2psBeginPage(const char* title,
  ...
  if (!gl2ps->viewport[2] || !gl2ps->viewport[3]) {
    gl2psMsg(GL2PS_ERROR, "Incorrect viewport (x=%d, y=%d, width=%d, height=%d)", gl2ps->viewport[0],
...
GL2PSDLL_API GLint gl2psEndPage(void) {
  ...
  if (res != GL2PS_OVERFLOW) (gl2psbackends[gl2ps->format]->printFooter)();
...
GL2PSDLL_API GLint
gl2psTextOpt(const char* str, const char* fontname, GLshort fontsize, GLint alignment, GLfloat angle) {
...
GL2PSDLL_API GLint gl2psDrawPixels(
    GLsizei width, GLsizei height, GLint xorig, GLint yorig, GLenum format, GLenum type, const void* pixels) {
  int size, i;
  GLfloat pos[4], *piv;
  GL2PSprimitive* prim;
  GLboolean valid;
...
GL2PSDLL_API GLint gl2psBlendFunc(GLenum sfactor, GLenum dfactor) {
  ...
  if (GL_FALSE == gl2psSupportedBlendMode(sfactor, dfactor)) return GL2PS_WARNING;
