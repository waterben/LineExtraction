...
std::vector<std::string> PlotManager::nameList() const {
  ...
  for_each(fmap_.begin(), fmap_.end(),
           [&](const std::pair<std::string, PlotWindow*>& data) { ret.push_back(data.first); });
  return ret;
}
