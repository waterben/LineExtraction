...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
  ...
  ui->customPlot->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom | QCP::iSelectAxes | QCP::iSelectLegend |
...
void MainWindow::titleDoubleClick(QMouseEvent* event, QCPPlotTitle* title) {
  ...
  bool ok;
  QString newTitle =
      QInputDialog::getText(this, "QCustomPlot example", "New plot title:", QLineEdit::Normal, title->text(), &ok);
  if (ok) {
...
void MainWindow::axisLabelDoubleClick(QCPAxis* axis, QCPAxis::SelectablePart part) {
      ...
      QCPAxis::spAxisLabel)  // only react when the actual axis label is clicked, not tick label or axis backbone
  {
    bool ok;
    QString newLabel =
        QInputDialog::getText(this, "QCustomPlot example", "New axis label:", QLineEdit::Normal, axis->label(), &ok);
    if (ok) {
...
void MainWindow::legendDoubleClick(QCPLegend* legend, QCPAbstractLegendItem* item) {
  ...
  Q_UNUSED(legend)
  if (item)  // only react if item was clicked (user could have clicked on border padding of legend where there is no
             // item, then item is 0)
  {
...
void MainWindow::contextMenuRequest(QPoint pos) {
  ...
  } else  // general context menu on graphs requested
  {
    ...
    if (ui->customPlot->graphCount() > 0) menu->addAction("Remove all graphs", this, SLOT(removeAllGraphs()));
  }
