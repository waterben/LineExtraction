...
CVPerformanceTestPtr createGradientPerformanceTest(const DataProviderList& provider) {
  ...
  test->tasks.push_back(std::make_shared<GradientEntry<short, int, float>>(
  ...
  // Gaussian
  test->tasks.push_back(std::make_shared<GradientEntry<float, float, float>>(
  ...
  test->tasks.push_back(std::make_shared<GradientEntry<float, float, float>>(
      std::make_shared<GradientPC<PCSqf<uchar, float, PolarCV>>>(
          IL{NV("grad_scale", 1), NV("grad_muls", 2), NV("grad_kernel_spacing", 1.2)}),
      "PC POF"));
  test->tasks.push_back(std::make_shared<GradientEntry<double, double, double>>(
