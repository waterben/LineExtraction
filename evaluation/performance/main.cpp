...
void setPriority() {
    if (setpriority(PRIO_PROCESS, getpid(), -NZERO) != 0)
        std::cout << "Failed to change priority" << std::endl;
    else
        std::cout << "Priority changed successfully!" << std::endl;
}
...
void addPerformanceTest(PerformanceTestPtr test) {
...
void addDefault(const DataProviderMap& provider, DataProviderList& data) {
    // simply add all
    for_each(provider.begin(), provider.end(), [&](const DataProviderMap::value_type& p) {
...
int main(int argc, char** argv)
{
    ...
    std::cout << "Total time for performance tests: " << static_cast<double>((cv::getTickCount() - start)) / cv::getTickFrequency() << "s" << std::endl;
