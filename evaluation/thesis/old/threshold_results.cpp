...
void processPath(std::vector<Entry>& entries, const std::pair<fs::path, std::string>& path) {
  ...
  std::for_each(files.begin(), files.end(), [&](const fs::path& file) {
    ...
    for_each(entries.begin(), entries.end(), [&](Entry& e) {
      ...
      saveResults(fs, e.processThreshold(fs["mag"].data),
                  path.second + "/" + file.stem().generic_string() + "_" + e.name);
    });
  });
}
...
int main() {
  ...
  filter.push_back(Entry(new DerivativeGradient<uchar, short, FT, FT, SobelDerivative>,
