...
void saveEdge(const cv::Mat& data, const std::string& name) { cv::imwrite("./nms/" + name + ".png", createNMS(data)); }

void saveResults(const lsfm::FilterResults& results,
...
int main() {
  ...
  filter.push_back(
      Entry(new DerivativeGradient<uchar, FT, FT, FT, SobelDerivative>({NV("grad_kernel_size", 7)}), "Sobel_(7x7)"));
  filter.push_back(
      Entry(new DerivativeGradient<uchar, FT, FT, FT, SobelDerivative>({NV("grad_kernel_size", 9)}), "Sobel_(9x9)"));
  ...
  filter.push_back(
      Entry(new RCMGradient<uchar, 3, short, FT, FT>(3, 1, cv::NORM_L2), "RCMG_(3x3)", ENTRY_NO_3 | ENTRY_RGB));
