...
/// @param Evaluates accuracy of gradient orientation for various gradient operators
/// @param Compares orientation histograms and phase information across different filters
...
struct Entry {
  Entry() : flags(0) {}
  Entry(const cv::Ptr<GradientI<uchar, GT, MT, DT>>& a, const std::string& b, int f = 0) : grad(a), name(b), flags(f) {}

  cv::Ptr<GradientI<uchar, GT, MT, DT>> grad;
  std::string name;
  int flags;

  inline bool hr() const { return flags & ENTRY_HALF_RANGE; }
...
double error(const cv::Mat& gt, const cv::Mat& dir, bool hr = false) {
  ...
  if (stepsi < 0)
    ...
    oss << std::setfill('0') << std::setw(4) << stepsi << "-";
...
double testBox(Entry<GT, MT, DT>& e, cv::Mat img, cv::Mat mask, double blur = 0, double noise = 0) {
  ...
  double sum = 0;
  for (stepsi = 0; stepsi <= steps; ++stepsi) sum += testSingle(e, img, mask, stepsi * CV_PI / (4 * steps), noise);
...
double testDisk(Entry<GT, MT, DT>& e, cv::Mat img, cv::Mat mask, double blur = 0, double noise = 0) {
  ...
  // imshow(e.name + " - grad", quiverDir<DT, MT>(res, e.grad->direction(), e.grad->directionRange(),
  // e.grad->magnitude(), 1, 1, 4, 1, 0.5));
#endif
...
int main() {
  ...
  gradF.push_back(
      Entry<double, double>(new DerivativeGradient<uchar, double, double, double, RobertsDerivative>, "Roberts (2x2)"));
  gradF.push_back(
      Entry<double, double>(new DerivativeGradient<uchar, double, double, double, PrewittDerivative>, "Prewitt (3x3)"));
  gradF.push_back(
      Entry<double, double>(new DerivativeGradient<uchar, double, double, double, ScharrDerivative>, "Scharr (3x3)"));

  gradF.push_back(
      Entry<double, double>(new DerivativeGradient<uchar, double, double, double, SobelDerivative>, "Sobel (3x3)"));
  gradF.push_back(Entry<double, double>(
      new DerivativeGradient<uchar, double, double, double, SobelDerivative>({NV("grad_kernel_size", 5)}),
      "Sobel (5x5)"));
  gradF.push_back(Entry<double, double>(
      new DerivativeGradient<uchar, double, double, double, SobelDerivative>({NV("grad_kernel_size", 7)}),
      "Sobel (7x7)"));
  gradF.push_back(Entry<double, double>(
      new DerivativeGradient<uchar, double, double, double, SobelDerivative>({NV("grad_kernel_size", 9)}),
      "Sobel (9x9)"));

  gradF.push_back(Entry<double, double>(new DerivativeGradient<uchar, double, double, double, GaussianDerivative>(
  ...
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureG2<uchar, double>>({NV("grad_kernel_size", 3), NV("grad_kernel_spacing", 1.24008)}),
      "QF StG (3x3)", ENTRY_HALF_RANGE));
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureG2<uchar, double>>({NV("grad_kernel_size", 5), NV("grad_kernel_spacing", 1.008)}),
      "QF StG (5x5)", ENTRY_HALF_RANGE));
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureG2<uchar, double>>({NV("grad_kernel_size", 7), NV("grad_kernel_spacing", 0.873226)}),
      "QF StG (7x7)", ENTRY_HALF_RANGE));
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureG2<uchar, double>>({NV("grad_kernel_size", 9), NV("grad_kernel_spacing", 0.781854)}),
      "QF StG (9x9)", ENTRY_HALF_RANGE));

  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureS<uchar, double, double>>(
          {NV("grad_scale", 1), NV("grad_muls", 2), NV("grad_kernel_size", 3), NV("grad_kernel_spacing", 1.2)}),
      "SQF Po (3x3)"));
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureS<uchar, double, double>>(
  ...
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureS<uchar, double, double>>(
          {NV("grad_scale", 1), NV("grad_muls", 2), NV("grad_kernel_size", 7), NV("grad_kernel_spacing", 1.2)}),
      "SQF Po (7x7)"));
  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureS<uchar, double, double>>(
          {NV("grad_scale", 1), NV("grad_muls", 2), NV("grad_kernel_size", 9), NV("grad_kernel_spacing", 1.2)}),
      "SQF Po (9x9)"));

  gradF.push_back(Entry<double, double>(
      new GradientOdd<QuadratureLGF<uchar, double>>({NV("grad_waveLength", 5), NV("grad_sigmaOnf", 0.55)}), "SQFF LG"));
  gradF.push_back(Entry<double, double>(new GradientOdd<QuadratureSF<uchar, double>>(
                                            {NV("grad_scale", 1), NV("grad_muls", 2), NV("grad_kernel_spacing", 1.2)}),
                                        "SQFF Po12"));
  gradF.push_back(Entry<double, double>(new GradientOdd<QuadratureSF<uchar, double>>(
                                            {NV("grad_scale", 1), NV("grad_muls", 3), NV("grad_kernel_spacing", 2.5)}),
                                        "SQFF Po"));

  gradF.push_back(Entry<double, double>(
      new GradientEnergy<PCLgf<uchar, double>>({NV("grad_waveLength", 3), NV("grad_sigmaOnf", 0.55)}), "PCF Lg"));
  gradF.push_back(Entry<double, double>(
  ...
  for_each(table_box.begin(), table_box.end(), [&](std::vector<std::string>& row) { row.resize(rows); });

  for_each(table_disk.begin(), table_disk.end(), [&](std::vector<std::string>& row) { row.resize(rows); });
  ...
  for_each(gradI.begin(), gradI.end(), [&](Entry<short>& e) {
    ...
    fname = "box-" + e.name;
    {
      ...
      oss << std::setprecision(3) << testBox(e, box.clone(), box_mask.clone(), 0, 0);
      table_box[row][1] = oss.str();
    }
    fname = "disk-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testDisk(e, disk.clone(), disk_mask.clone(), 0, 0);
      table_disk[row++][1] = oss.str();
    }
  });
  ...
  for_each(gradI.begin(), gradI.end(), [&](Entry<short>& e) {
    fname = "box-blur" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testBox(e, box.clone(), box_mask.clone(), 2, 0);
      table_box[row][2] = oss.str();
    }
    fname = "disk-blur" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testDisk(e, disk.clone(), disk_mask.clone(), 2, 0);
      table_disk[row++][2] = oss.str();
    }
  });
  for_each(gradF.begin(), gradF.end(), [&](Entry<double, double>& e) {
    fname = "box-blur-" + e.name;
  ...
  for_each(gradI.begin(), gradI.end(), [&](Entry<short>& e) {
    fname = "box-noise10-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testBox(e, box.clone(), box_mask.clone(), 0, 10);
      table_box[row][3] = oss.str();
    }
    fname = "disk-noise10-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testDisk(e, disk.clone(), disk_mask.clone(), 0, 10);
      table_disk[row++][3] = oss.str();
    }
  });
  for_each(gradF.begin(), gradF.end(), [&](Entry<double, double>& e) {
    fname = "box-noise10-" + e.name;
  ...
  for_each(gradI.begin(), gradI.end(), [&](Entry<short>& e) {
    fname = "box-noise40-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testBox(e, box.clone(), box_mask.clone(), 0, 40);
      table_box[row][4] = oss.str();
    }
    fname = "disk-noise40-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testDisk(e, disk.clone(), disk_mask.clone(), 0, 40);
      table_disk[row++][4] = oss.str();
    }
  });
  for_each(gradF.begin(), gradF.end(), [&](Entry<double, double>& e) {
    fname = "box-noise40-" + e.name;
  ...
  for_each(gradI.begin(), gradI.end(), [&](Entry<short>& e) {
    fname = "box-bnoise10-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testBox(e, box.clone(), box_mask.clone(), 2, 10);
      table_box[row][5] = oss.str();
    }
    fname = "disk-bnoise10-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testDisk(e, disk.clone(), disk_mask.clone(), 2, 10);
      table_disk[row++][5] = oss.str();
    }
  });
  for_each(gradF.begin(), gradF.end(), [&](Entry<double, double>& e) {
    fname = "box-bnoise10-" + e.name;
  ...
  for_each(gradI.begin(), gradI.end(), [&](Entry<short>& e) {
    fname = "box-bnoise40-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testBox(e, box.clone(), box_mask.clone(), 2, 40);
      table_box[row][6] = oss.str();
    }
    fname = "disk-bnoise40-" + e.name;
    {
      std::ostringstream oss;
      oss.setf(std::ios::fixed);
      oss << std::setprecision(3) << testDisk(e, disk.clone(), disk_mask.clone(), 2, 40);
      table_disk[row++][6] = oss.str();
    }
  });
  for_each(gradF.begin(), gradF.end(), [&](Entry<double, double>& e) {
    fname = "box-bnoise40-" + e.name;
